...
public:
  ...
  template <typename T = float> static T softplus(T x) {
    ...
    return static_cast<T>(log(1 + exp_util<T>((T)beta * x)) / beta);
  }
  ...
  template <typename T = float> static T elu(T x) {
    return x >= static_cast<T>(0.0) ? x : static_cast<T>(alpha) * (T(exp(x)) - 1);
  }
