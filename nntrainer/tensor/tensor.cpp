...
Tensor Tensor::naive_dot(Tensor const &m, bool trans, bool trans_m) const {
  Tensor output("", this->getFormat(), this->getDataType());
  naive_dot(m, output, trans, trans_m);
...
Tensor &Tensor::naive_dot(Tensor const &m, Tensor &result, bool trans, bool trans_m,
                    float beta) const {
  NNTR_THROW_IF(!contiguous, std::invalid_argument)
    << getName() << " is not contiguous. Cannot dot product.";
  ...
  if (getDataType() == ml::train::TensorDim::DataType::FP32) {
    ...
    /// case others: use sgemm
      ...
      naive_sgemm(CblasRowMajor, transA, transB, M, N, K, alpha, data, lda, mdata,
            ldb, beta, rdata, ldc);
    }
#else
    throw std::invalid_argument("Error: enable-fp16 is not enabled");
