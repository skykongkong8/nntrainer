...
// void packing_a_v9_2(__fp16 *src, __fp16 *dst, int leading_dim, int dim_first,
//                     int dim_second) {
...
// void packing_b_v9_2(__fp16 *src, __fp16 *dst, int leading_dim, int dim_first,
//                     int dim_second) {
...
// static inline void hgemmv9_2_opt(unsigned int M, unsigned int N, unsigned int K,
...
// void hgemmv9_2_macro_kernel(unsigned int M, unsigned int N, unsigned int K,
...
//   unsigned int M_ = M & -VL_FP16_DOUBLE, N_ = N & -VL_FP16_HALF, K_ = K & -VL_FP16_HALF;
//   auto ptr_packing_a = A;
//   auto ptr_packing_b = B;
//   /// @todo do for alpha
// float16x8_t valpha = vdupq_n_f16(1.f); // broadcast alpha to a 256-bit
//   //   vector
//   float16x8_t a0, a1;
//   float16x8_t b0, b1, b2, b3;
//   float16x8_t c00, c01, c20, c21;
//   float16x8_t c10, c11, c30, c31;

//   for (i = 0; i < M_; i += VL_FP16_DOUBLE) {
//     for (j = 0; j < N_; j += VL_FP16_HALF) {
//       ptr_packing_a = A + i * K;
//       ptr_packing_b = B + j * K;
//       macro_KERNEL_16xkx4_HGEMM_packing();
//     }
//   }
...
//       ptr_packing_a = A + i * K;
//       ptr_packing_b = B + j * K;
//       macro_KERNEL_8xkx4_HGEMM_packing_v2();
//     }
//   }
// }

// void hgemmv9_2(unsigned int M, unsigned int N, unsigned int K, float alpha,
...
//       packing_b_v9_2(B + k_count + n_count * LDB, b_buffer, LDB, k_inc, n_inc);
//       for (m_count = 0; m_count < M; m_count += m_inc) {
//         m_inc = (M - m_count > M_BLOCKING) ? M_BLOCKING : N - m_count;
//         packing_a_v9_2(A + m_count + k_count * LDA, a_buffer, LDA, m_inc,
//                        k_inc);
//         // macro kernel: to compute C += A_tilt * B_tilt
//         hgemmv9_2_macro_kernel(m_inc, n_inc, k_inc, alpha, &A(m_count, k_count),
