...
#define HGEMM_KERNEL_8x24 hgemm_kernel_8x24


void hgemm_noTrans(const __fp16 *A, const __fp16 *B, float *C32, unsigned int M,
                   unsigned int N, unsigned int K, float alpha, float beta) {
  if (alpha == 1.F && beta == 0.F) {
    if (M % 8 == 0 && N % 24 == 0 && K % 8 == 0) {
      hgemm_noTrans_8x24(M, N, K, A, K, B, N, C32, N, alpha, beta);
    }
    else
...
void hgemm_noTrans_8x24(unsigned int M, unsigned int N, unsigned int K,
                        const __fp16 *A, unsigned int lda, const __fp16 *B,
                        unsigned int ldb, __fp16 *C, unsigned int ldc,
                        float alpha, float beta) {
  ...
  for (ms = 0; ms < M; ms += M_BLOCKING) {
    ...
    for (ks = 0; ks < K; ks += k_min) {
      ...
      if (N >= N_BLOCKING * 2) {
        ...
        n_min = ((n_min / 2 + GEMM_UNROLLING_24 - 1) / GEMM_UNROLLING_24) *
                GEMM_UNROLLING_24;
      } else {
        stride_l1 = 0;
      }
      packing_B24(k_min, n_min, B + ks * ldb, ldb, sB);
      ...
      for (ms2 = ms; ms2 < ms + m_min; ms2 += m2_min) {
        ...
        HGEMM_KERNEL_8x24(m2_min, n_min, k_min,
                          sA + stride_l1 * k_min * (ms2 - ms), sB,
                          C + ms2 * ldc, ldc);
      }
      ...
      for (ns = n_min; ns < N; ns += n_min) {
        ...
        packing_B24(k_min, n_min, B + ns + ldb * ks, ldb, sB);
        HGEMM_KERNEL_8x24(m_min, n_min, k_min, sA, sB, C + ms * ldc + ns, ldc);
      }
    }
  }
  ...
  for (ms = 0; ms < M; ms += M_BLOCKING) {
    ...
    for (ks = 0; ks < K; ks += k_min) {
      ...
      for (ms2 = ms; ms2 < ms + m_min; ms2 += m2_min) {
        ...
        HGEMM_KERNEL_8x24(m2_min, n_min, k_min, sA + k_min * (ms2 - ms), sB,
                          C + ms2 * ldc, ldc);
      }
