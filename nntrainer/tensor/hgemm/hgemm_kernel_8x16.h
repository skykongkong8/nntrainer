...
#define KERNEL_8x16_ACC1_ALPHA6()                    \
  ...
  v112_119 = vfmaq_laneq_f16(v112_119, v25, va0, 6); \
  v120_127 = vfmaq_laneq_f16(v120_127, v25, va0, 7); \
  va1 = vmulq_f16(vld1q_f16(a + 8), valpha);         \
  v26 = vld1q_f16(b + 8 * 2);                        \
  v27 = vld1q_f16(b + 8 * 3);                        \
  v0_7 = vfmaq_laneq_f16(v0_7, v26, va1, 0);         \
  v8_15 = vfmaq_laneq_f16(v8_15, v26, va1, 1);       \
 ...
 */
void hgemm_kernel_8x16_wo_alpha(unsigned int M, unsigned int N, unsigned int K,
                                __fp16 *sa, __fp16 *sb, float *sc,
 ...
 * @param beta scale factor
 */
void hgemm_kernel_8x16_w_alpha(unsigned int M, unsigned int N, unsigned int K,
                               __fp16 *sa, __fp16 *sb, float *sc,
                               unsigned int ldc, float alpha, float beta) {
  assert(M > 0 && N > 0 && K > 0);
  assert(M % 8 == 0 && N % 16 == 0);
...
void hgemm_kernel_8x16(unsigned int M, unsigned int N, unsigned int K,
  ...
  if (alpha == 1.F)
    hgemm_kernel_8x16_wo_alpha(M, N, K, sa, sb, sc, ldc, alpha, beta);
  else
    hgemm_kernel_8x16_w_alpha(M, N, K, sa, sb, sc, ldc, alpha, beta);
}
