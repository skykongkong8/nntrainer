...
static inline void __ggml_q4_0_16x8_q8_0_GEMM_GEMV(
  const unsigned int M, const unsigned int N, const unsigned int K,
  const float *A, const unsigned int lda, const void *B, const unsigned int ldb,
  float *C, const unsigned int ldc) {
  int blocks_per_row = (K + QK8_0 - 1) / QK8_0;
  ...
  BS::multi_future<void> loop_future =
    bs_thread_pool.submit_loop(0, thread_num, [=](int i) {
      ...
      M_step_start = (M_step_start % 16) ? M_step_start + 16 - (M_step_start % 16)
                                        : M_step_start;
      M_step_end =
        (M_step_end % 16) ? M_step_end + 16 - (M_step_end % 16) : M_step_end;

      nntr_gemv_q4_0_16x8_q8_0(K, (float *)(C + M_step_start), N,
                              (void *)((char *)B + M_step_start * B_step),
                              QA.data(), M, M_step_end - M_step_start);
    });
  loop_future.wait();
...
static inline void __ggml_q4_0_16x8_q8_0_GEMM_GEMM(
  const unsigned int M, const unsigned int N, const unsigned int K,
  const float *A, const unsigned int lda, const void *B, const unsigned int ldb,
  float *C, const unsigned int ldc) {
  auto &bs_thread_pool = ThreadPoolManager::Global().getThreadPool();
  ...
  BS::multi_future<void> multi_future =
    bs_thread_pool.submit_loop(0, thread_num, [=](int i) {
      ...
      nntr_gemm_q4_0_16x8_q8_0(K, (C + (M_step_start)), ldc,
                              ((char *)B + ((M_step_start)*B_step)), QA.data(),
                              M4 * 4, (M_step_end) - (M_step_start));
    });
  multi_future.wait();
  ...
  for (unsigned int pb = M4 * 4; pb < M; pb++) {
    BS::multi_future<void> loop_future =
      bs_thread_pool.submit_loop(0, thread_num, [=](int i) {
        ...
        M_step_start = (M_step_start % 16)
                         ? M_step_start + 16 - (M_step_start % 16)
                         : M_step_start;
        M_step_end =
          (M_step_end % 16) ? M_step_end + 16 - (M_step_end % 16) : M_step_end;

        nntr_gemv_q4_0_16x8_q8_0(
          K, (float *)((C + ((pb - M4 * 4) * N) + (M4 * 4 * N)) + M_step_start),
          N, (void *)((char *)B + M_step_start * B_step),
          QA.data() + (M4 * qa_4_rows_size) + (pb - M4 * 4) * qa_row_size, 1,
          M_step_end - M_step_start);
...
void __ggml_q4_0_16x8_q8_0_GEMM(const unsigned int M, const unsigned int N,
                               const unsigned int K, const float *A,
                               const unsigned int lda, const void *B,
                               const unsigned int ldb, float *C,
                               const unsigned int ldc) {
  if (M == 1) { // GEMV
    __ggml_q4_0_16x8_q8_0_GEMM_GEMV(M, N, K, A, lda, B, ldb, C, ldc);
  } else { // GEMM
    __ggml_q4_0_16x8_q8_0_GEMM_GEMM(M, N, K, A, lda, B, ldb, C, ldc);
  }
}
