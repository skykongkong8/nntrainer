...
void uhgemm(const uint16_t *A, const uint16_t *B, uint16_t *C, unsigned int M,
  ...
  unsigned int *C32 =
    (unsigned int *)malloc(M8_high * N16_high * sizeof(unsigned int));
  ...
  for (unsigned int m = 0; m < M; ++m) {
    for (unsigned int n = 0; n < N8_low; n += 8) {
      ...
      vst1q_u16(&C[m * N + n], vcombine_u16(vmovn_u32(x1), vmovn_u32(x2)));
    }
    for (unsigned int n = N8_low; n < N; ++n) {
...
void uhgemm_ensure_divisibility(const uint16_t *A, const uint16_t *B,
  ...
  if ((M8_high != M) || (K8_high != K)) {
    pad_A = true;
    Ap = alignedMalloc<uint16_t>(M8_high * K8_high);
  ...
  if ((K8_high != K) || (N16_high != N)) {
    pad_B = true;
    Bp = alignedMalloc<uint16_t>(K8_high * N16_high);
