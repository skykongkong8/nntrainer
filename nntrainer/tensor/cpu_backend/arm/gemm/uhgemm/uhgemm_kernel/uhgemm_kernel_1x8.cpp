// SPDX-License-Identifier: Apache-2.0
/**
 * Copyright (C) 2024 Sungsik Kong <ss.kong@samsung.com>
 *
 * @file   uhgemm_kernel_1x8.cpp
 * @date   05 August 2024
 * @see    https://github.com/nnstreamer/nntrainer
 * @author Sungsik Kong <ss.kong@samsung.com>
 * @author Sungsik Kong <ss.kong@samsung.com>
 * @bug    No known bugs except for NYI items
 * @brief  This is uint16 GEMM 1x8 kernel
 *
 */

#include <arm_neon.h>
#include <assert.h>
#include <uhgemm_kernel.h>
#include <gemm_util.h>
#include <stdlib.h>

// 1. Partial sum 64 digits : worst accuracy, best latency
#define KERNEL_1x8_ACC8()           \
  do {                              \
    v0 = vdupq_n_u16(0);          \
    dv0 = *a;                       \
    v24 = vld1q_u16(b);             \
    v0 = vmlaq_n_u16(v0, v24, dv0); \
    dv1 = *(a + 1);                 \
    v25 = vld1q_u16(b + 8);         \
    v0 = vmlaq_n_u16(v0, v25, dv1); \
    dv2 = *(a + 2);                 \
    v26 = vld1q_u16(b + 16);        \
    v0 = vmlaq_n_u16(v0, v26, dv2); \
    dv3 = *(a + 3);                 \
    v27 = vld1q_u16(b + 24);        \
    v0 = vmlaq_n_u16(v0, v27, dv3); \
    dv4 = *(a + 4);                 \
    v28 = vld1q_u16(b + 32);        \
    v0 = vmlaq_n_u16(v0, v28, dv4); \
    dv5 = *(a + 5);                 \
    v29 = vld1q_u16(b + 40);        \
    v0 = vmlaq_n_u16(v0, v29, dv5); \
    dv6 = *(a + 6);                 \
    v30 = vld1q_u16(b + 48);        \
    v0 = vmlaq_n_u16(v0, v30, dv6); \
    dv7 = *(a + 7);                 \
    v31 = vld1q_u16(b + 56);        \
    v0 = vmlaq_n_u16(v0, v31, dv7); \
    l += 8;                         \
    b += 8 * 8;                     \
    a += 8;                         \
  } while (0)

// 2. Partial sum 32 digits : medium accuracy, medium latency
#define KERNEL_1x8_ACC4()           \
  do {                              \
    v0 = vdupq_n_u16(0);          \
    dv0 = *a;                       \
    v24 = vld1q_u16(b);             \
    v0 = vmlaq_n_u16(v0, v24, dv0); \
    dv1 = *(a + 1);                 \
    v25 = vld1q_u16(b + 8);         \
    v0 = vmlaq_n_u16(v0, v25, dv1); \
    dv2 = *(a + 2);                 \
    v26 = vld1q_u16(b + 16);        \
    v0 = vmlaq_n_u16(v0, v26, dv2); \
    dv3 = *(a + 3);                 \
    v27 = vld1q_u16(b + 24);        \
    v0 = vmlaq_n_u16(v0, v27, dv3); \
    l += 4;                         \
    b += 8 * 4;                     \
    a += 4;                         \
  } while (0)

// 3. Partial sum 8 digits : Best accuracy, worst latency
#define KERNEL_1x8_ACC1()           \
  do {                              \
    v0 = vdupq_n_u16(0);          \
    dv0 = *(a);                     \
    v24 = vld1q_u16(b);             \
    v0 = vmlaq_n_u16(v0, v24, dv0); \
    l += 1;                         \
    b += 8 * 1;                     \
    a++;                            \
  } while (0)

template <>
void uhgemm_kernel_1x8(unsigned int M, unsigned int N, unsigned int K,
                      uint16_t *sa, uint16_t *sb, uint16_t *sc, unsigned int ldc) {
  assert(M > 0 && N > 0 && K > 0);
  assert(N % 8 == 0);

  uint16_t *a = sa, *b = sb, *c = sc;
  unsigned int k8 = (K >> 3) << 3;
  unsigned int i, j, l;
  for (i = 0; i < M; i++) {
    for (j = 0; j < N; j += 8) {
      __builtin_prefetch(b, 0, 3);
      __builtin_prefetch(a, 0, 3);
      uint16x8_t v0;
      uint16x8_t v24, v25, v26, v27, v28, v29, v30, v31;
      float16_t dv0, dv1, dv2, dv3, dv4, dv5, dv6, dv7;
      l = 0;
      for (; l < k8;) {
        KERNEL_1x8_ACC8();

        vst1q_u16(c, vaddq_u16(vld1q_u16(c), v0));
      }
      for (; l < K;) {
        KERNEL_1x8_ACC1();

        vst1q_u16(c, vaddq_u16(vld1q_u16(c), v0));
      }
      c += 8;
      a -= K;
    }
    sc += ldc;
    c = sc;
    a += K;
    b = sb;
  }
}

template <>
void uhgemm_kernel_1x8(unsigned int M, unsigned int N, unsigned int K,
                      uint16_t *sa, uint16_t *sb, unsigned int *sc, unsigned int ldc) {
  assert(M > 0 && N > 0 && K > 0);
  assert(N % 8 == 0);

  uint16_t *a = sa, *b = sb;
  unsigned int *c = sc;
  unsigned int K8 = get_prev_mltpl_of_2p_n(K, 3);
  unsigned int i, j, l;
  for (i = 0; i < M; i++) {
    for (j = 0; j < N; j += 8) {
      __builtin_prefetch(b, 0, 3);
      __builtin_prefetch(a, 0, 3);
      uint16x8_t v0;
      uint16x8_t v24, v25, v26, v27, v28, v29, v30, v31;
      float16_t dv0, dv1, dv2, dv3, dv4, dv5, dv6, dv7;
      l = 0;
      for (; l < K8;) {
        KERNEL_1x8_ACC8();

        vst1q_u32(c, vaddq_u32(vld1q_u32(c), vmovl_u16(vget_low_u16(v0))));

        vst1q_u32(c + 4,
                  vaddq_u32(vld1q_u32(c + 4), vmovl_u16(vget_high_u16(v0))));
      }
      for (; l < K;) {
        KERNEL_1x8_ACC1();

        vst1q_u32(c, vaddq_u32(vld1q_u32(c), vmovl_u16(vget_low_u16(v0))));

        vst1q_u32(c + 4,
                  vaddq_u32(vld1q_u32(c + 4), vmovl_u16(vget_high_u16(v0))));
      }
      c += 8;
      a -= K;
    }
    sc += ldc;
    c = sc;
    a += K;
    b = sb;
  }
}
