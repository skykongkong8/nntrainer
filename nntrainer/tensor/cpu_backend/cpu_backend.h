 ...
 */
extern _FP16 max(const unsigned int N, _FP16 *X);
 ...
 */
extern bool is_valid(const size_t N, const _FP16 *X);
 ...
 */
extern float max(const unsigned int N, float *X);
 ...
 * @param[out] bool false if not valid else true
 */
extern bool is_valid(const size_t N, const float *X);
#endif
#endif
