...
MlasGemmQuantFixupZeroPointA<MLAS_GEMM_QUANT_KERNEL_DEFAULT>(int32_t ZeroPointA,
                                                             bool AIsSigned) {
  if (AIsSigned) {
...
template <>
inline constexpr int32_t
MlasGemmQuantFixupZeroPointB<MLAS_GEMM_QUANT_KERNEL_DEFAULT>(int32_t ZeroPointB,
                                                             bool BIsSigned) {
  if (BIsSigned) {
...
template <>
void MlasGemmQuantCopyPackA<MLAS_GEMM_QUANT_KERNEL_DEFAULT>(
  MLAS_GEMM_QUANT_KERNEL_DEFAULT::PackedAType *D, const uint8_t *A, size_t lda,
  size_t CountM, size_t CountK, int32_t *RowSumBuffer, bool AIsSigned) {
  const size_t AlignedCountK =
    (CountK + MLAS_GEMM_QUANT_KERNEL_DEFAULT::PackedK - 1) &
...
template <>
void MlasGemmQuantCopyPackB<MLAS_GEMM_QUANT_KERNEL_DEFAULT>(
  MLAS_GEMM_QUANT_KERNEL_DEFAULT::PackedBType *D, const uint8_t *B, size_t ldb,
  size_t CountN, size_t CountK, int32_t *ColumnSumBuffer, bool BIsSigned) {
  const size_t AlignedCountK =
    (CountK + MLAS_GEMM_QUANT_KERNEL_DEFAULT::PackedK - 1) &
    ~(MLAS_GEMM_QUANT_KERNEL_DEFAULT::PackedK - 1);
...
template <>
size_t MlasGemmQuantKernel<MLAS_GEMM_QUANT_KERNEL_DEFAULT>(
  const MLAS_GEMM_QUANT_KERNEL_DEFAULT::PackedAType *A,
  const MLAS_GEMM_QUANT_KERNEL_DEFAULT::PackedBType *B, int32_t *C,
