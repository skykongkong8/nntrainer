...
#include <arm_neon.h>
  ...
  !defined(signature_AMD_ebx) // workaround for Bug 96238 - [i386] cpuid.h
                              // header needs include guards
#include <cpuid.h>
#endif
...
inline void MlasPrintFinalMessage(const std::string &msg) {
#if defined(__ANDROID__)
  ...
  // logging when the exceptions are disabled, since using std::cerr might
  // increase binary size, and std::cerr output might not be easily accesible on
  // some systems such as mobile
  // TODO, see if we need to change the output of the error message from
  // std::cerr to NSLog for iOS
...
typedef size_t(MLASCALL MLAS_GEMM_DOUBLE_KERNEL)(
  const double *A, const double *B, double *C, size_t CountK, size_t CountM,
  size_t CountN, size_t lda, size_t ldc, double alpha, bool ZeroMode);
...
// );
typedef size_t(MLASCALL MLAS_SBGEMM_FLOAT_KERNEL)(
  const float *A, const uint16_t *B, float *C, size_t CountK, size_t CountM,
  size_t CountN, size_t lda, size_t ldc, const float *Bias);
#endif
...
typedef void(MLASCALL MLAS_SGEMM_KERNEL_M1_ROUTINE)(const float *A,
...
typedef size_t(MLASCALL MLAS_GEMV_U8S8_KERNEL)(const uint8_t *A,
                                               const uint8_t *B, int32_t *C,
                                               size_t CountK, size_t CountN,
                                               size_t ldb);
...
typedef void(MLASCALL MLAS_CONV_POINTWISE_FLOAT_KERNEL)(
  ...
  size_t FilterStride, size_t OutputStride, size_t OutputCount,
...
typedef void(MLASCALL MLAS_REDUCE_MINIMUM_MAXIMUM_FLOAT_KERNEL)(
  const float *Input, float *Min, float *Max, size_t N);

typedef void(MLASCALL MLAS_CAST_F16_TO_F32_KERNEL)(const unsigned short *Source,
                                                   float *Destination,
                                                   size_t Count);

typedef void(MLASCALL MLAS_CAST_F32_TO_F16_KERNEL)(const float *Source,
...
typedef void(MLASCALL MLAS_QLINEAR_BINARY_OP_U8_KERNEL)(
  ...
  const uint8_t *InputB, float ScaleB, int32_t ZeroPointB, float ScaleC,
  int32_t ZeroPointC, uint8_t *OutputC, size_t N, bool IsScalarB);
...
template <typename InputType, typename FilterType> struct MLAS_QUANT_KERNEL {
...
MLAS_COMPUTE_LOGSOFTMAX_OUTPUT_FLOAT_KERNEL
  MlasComputeLogSoftmaxOutputF32KernelLasx;
#else
...
MLAS_COMPUTE_LOGSOFTMAX_OUTPUT_FLOAT_KERNEL
  MlasComputeLogSoftmaxOutputF32KernelAvx;
...
void MlasSgemmOperation(CBLAS_TRANSPOSE TransA, CBLAS_TRANSPOSE TransB,
...
//
// Quantized 8-bit integer/quantized 4-bit integer matrix/matrix multiply
// dispatch structure.
//
...
template <typename InputType, typename FilterType>
void MLASCALL MlasConvDepthwiseKernelAvx2(
  const InputType *const *Input, InputType InputZeroPoint,
  const FilterType *Filter, FilterType FilterZeroPoint, int32_t *Output,
 ...
 *
 * @param ThreadPool [IN]          Optional thread pool. Ignored when using
 * OpenMP
 * @param Iterations [IN]          Total number of iterations
 * @param Work [IN]                Logic for computing a range of iterations
 * [begin, end)
 */
...
#pragma warning(push)
// VC++ suggests we can attempt to make 'MlasBitsOfFp32' constexpr, but it is
// not valid.
#pragma warning(disable : 26497)
#endif

inline uint32_t MlasBitsOfFp32(float FloatValue) {
  union {
...
inline float MlasFp32FromBits(uint32_t IntegerValue) {
  union {
    uint32_t IntegerValue;
    float FloatValue;
  } u;
...
inline MLAS_INT32X4 MlasReinterpretAsInt32x4(MLAS_FLOAT32X4 Vector) {
#if defined(MLAS_NEON_INTRINSICS)
...
inline MLAS_INT32X4 MlasCastToInt32x4(MLAS_FLOAT32X4 Vector) {
#if defined(MLAS_NEON_INTRINSICS)
...
#else
  return MLAS_INT32X4{int32_t(Vector[0]), int32_t(Vector[1]),
                      int32_t(Vector[2]), int32_t(Vector[3])};
#endif
}

inline MLAS_FLOAT32X4 MlasCastToFloat32x4(MLAS_INT32X4 Vector) {
#if defined(MLAS_NEON_INTRINSICS)
...
inline MLAS_INT32X4 MlasBroadcastInt32x4(int32_t Value) {
#if defined(MLAS_NEON_INTRINSICS)
...
inline MLAS_INT32X4 MlasLoadInt32x4(const int32_t *Buffer) {
#if defined(MLAS_NEON_INTRINSICS)
...
inline void MlasStoreInt32x4(int32_t *Buffer, MLAS_INT32X4 Vector) {
#if defined(MLAS_NEON_INTRINSICS)
...
inline MLAS_INT32X4 MlasAddInt32x4(MLAS_INT32X4 Vector1, MLAS_INT32X4 Vector2) {
#if defined(MLAS_NEON_INTRINSICS)
...
inline MLAS_INT32X4 MlasSubtractInt32x4(MLAS_INT32X4 Vector1,
                                        MLAS_INT32X4 Vector2) {
#if defined(MLAS_NEON_INTRINSICS)
...
inline MLAS_INT32X4 MlasAndInt32x4(MLAS_INT32X4 Vector1, MLAS_INT32X4 Vector2) {
#if defined(MLAS_NEON_INTRINSICS)
...
inline MLAS_INT32X4 MlasOrInt32x4(MLAS_INT32X4 Vector1, MLAS_INT32X4 Vector2) {
#if defined(MLAS_NEON_INTRINSICS)
...
inline MLAS_INT32X4 MlasAndNotInt32x4(MLAS_INT32X4 VectorNot,
                                      MLAS_INT32X4 Vector) {
#if defined(MLAS_NEON_INTRINSICS)
...
inline MLAS_INT32X4 MlasXorInt32x4(MLAS_INT32X4 Vector1, MLAS_INT32X4 Vector2) {
#if defined(MLAS_NEON_INTRINSICS)
...
inline MLAS_INT32X4 MlasBlendInt32x4(MLAS_INT32X4 Vector1, MLAS_INT32X4 Vector2,
...
inline MLAS_INT32X4 MlasShiftLeftInt32x4(MLAS_INT32X4 Vector) {
#if defined(MLAS_NEON_INTRINSICS)
...
inline MLAS_INT32X4 MlasMaximumInt32x4(MLAS_INT32X4 Vector1,
                                       MLAS_INT32X4 Vector2) {
#if defined(MLAS_NEON_INTRINSICS)
...
inline MLAS_INT32X4 MlasMinimumInt32x4(MLAS_INT32X4 Vector1,
                                       MLAS_INT32X4 Vector2) {
#if defined(MLAS_NEON_INTRINSICS)
...
inline MLAS_FLOAT32X4 MlasReinterpretAsFloat32x4(MLAS_INT32X4 Vector) {
#if defined(MLAS_NEON_INTRINSICS)
...
inline MLAS_FLOAT32X4 MlasBroadcastFloat32x4(float Value) {
#if defined(MLAS_NEON_INTRINSICS)
...
inline MLAS_FLOAT32X4 MlasBroadcastFloat32x4(const float *Value) {
#if defined(MLAS_NEON_INTRINSICS)
...
inline MLAS_FLOAT32X4 MlasZeroFloat32x4(void) {
#if defined(MLAS_NEON_INTRINSICS)
...
inline MLAS_FLOAT32X4 MlasLoadFloat32x4(const float *Buffer) {
#if defined(MLAS_NEON_INTRINSICS)
  ...
  return wasm_v128_load(Buffer);
#elif defined(MLAS_LSX_INTRINSICS)
  // return MlasReinterpretAsFloat32x4(__lsx_vld((const MLAS_INT32X4 *)Buffer,
  // 0));
...
inline void MlasStoreFloat32x4(float *Buffer, MLAS_FLOAT32X4 Vector) {
#if defined(MLAS_NEON_INTRINSICS)
...
inline void MlasStoreAlignedFloat32x4(float *Buffer, MLAS_FLOAT32X4 Vector) {
#if defined(MLAS_NEON_INTRINSICS)
  vst1q_f32(Buffer, Vector);
#elif defined(MLAS_SSE2_INTRINSICS)
...
inline void MlasStoreLaneFloat32x4(float *Buffer, MLAS_FLOAT32X4 Vector) {
#if defined(MLAS_NEON_INTRINSICS)
  ...
  _mm_store_ss(Buffer, _mm_shuffle_ps(Vector, Vector,
                                      _MM_SHUFFLE(Lane, Lane, Lane, Lane)));
#elif defined(MLAS_WASM_SIMD_INTRINSICS)
...
inline void MlasStoreLowHalfFloat32x4(float *Buffer, MLAS_FLOAT32X4 Vector) {
#if defined(MLAS_NEON_INTRINSICS)
...
template <unsigned Lane>
inline float MlasExtractLaneFloat32x4(MLAS_FLOAT32X4 Vector) {
#if defined(MLAS_NEON_INTRINSICS)
...
template <>
inline void MlasStoreLaneFloat32x4<0>(float *Buffer, MLAS_FLOAT32X4 Vector) {
  _mm_store_ss(Buffer, Vector);
}

template <> inline float MlasExtractLaneFloat32x4<0>(MLAS_FLOAT32X4 Vector) {
  return _mm_cvtss_f32(Vector);
}

template <unsigned Index0, unsigned Index1, unsigned Index2, unsigned Index3>
inline MLAS_FLOAT32X4 MlasShuffleFloat32x4(MLAS_FLOAT32X4 Vector) {
  return _mm_shuffle_ps(Vector, Vector,
                        _MM_SHUFFLE(Index3, Index2, Index1, Index0));
}
...
template <unsigned Index0, unsigned Index1, unsigned Index2, unsigned Index3>
inline MLAS_FLOAT32X4 MlasShuffleFloat32x4(MLAS_FLOAT32X4 Vector1,
  ...
  return __builtin_shufflevector(Vector1, Vector2, Index0, Index1, Index2,
                                 Index3);
#elif defined(MLAS_LSX_INTRINSICS)
...
inline MLAS_FLOAT32X4 MlasInterleaveLowFloat32x4(MLAS_FLOAT32X4 Vector1,
                                                 MLAS_FLOAT32X4 Vector2) {
...
inline MLAS_FLOAT32X4 MlasInterleaveHighFloat32x4(MLAS_FLOAT32X4 Vector1,
                                                  MLAS_FLOAT32X4 Vector2) {
#if defined(MLAS_NEON64_INTRINSICS)
...
inline MLAS_FLOAT32X4 MlasAddFloat32x4(MLAS_FLOAT32X4 Vector1,
                                       MLAS_FLOAT32X4 Vector2) {
#if defined(MLAS_NEON_INTRINSICS)
...
inline MLAS_FLOAT32X4 MlasSubtractFloat32x4(MLAS_FLOAT32X4 Vector1,
                                            MLAS_FLOAT32X4 Vector2) {
#if defined(MLAS_NEON_INTRINSICS)
...
inline MLAS_FLOAT32X4 MlasMultiplyFloat32x4(MLAS_FLOAT32X4 Vector1,
                                            MLAS_FLOAT32X4 Vector2) {
#if defined(MLAS_NEON_INTRINSICS)
...
inline MLAS_FLOAT32X4 MlasMultiplyAddFloat32x4(MLAS_FLOAT32X4 Vector1,
                                               MLAS_FLOAT32X4 Vector2,
...
inline MLAS_FLOAT32X4 MlasMultiplyAddFloat32x4(MLAS_FLOAT32X4 Vector1,
                                               float Scalar2,
                                               MLAS_FLOAT32X4 Vector3) {
  return MlasMultiplyAddFloat32x4(Vector1, MlasBroadcastFloat32x4(Scalar2),
                                  Vector3);
}
...
inline MLAS_FLOAT32X4 MlasMultiplyAddFloat32x4(MLAS_FLOAT32X4 Vector1,
                                               MLAS_FLOAT32X4 Vector2,
                                               float Scalar3) {
  return MlasMultiplyAddFloat32x4(Vector1, Vector2,
                                  MlasBroadcastFloat32x4(Scalar3));
}

inline MLAS_FLOAT32X4 MlasDivideFloat32x4(MLAS_FLOAT32X4 Vector1,
                                          MLAS_FLOAT32X4 Vector2) {
#if defined(MLAS_NEON64_INTRINSICS)
  return vdivq_f32(Vector1, Vector2);
#elif defined(MLAS_NEON32_INTRINSICS)
  Vector1 = vsetq_lane_f32(
    vgetq_lane_f32(Vector1, 0) / vgetq_lane_f32(Vector2, 0), Vector1, 0);
  Vector1 = vsetq_lane_f32(
    vgetq_lane_f32(Vector1, 1) / vgetq_lane_f32(Vector2, 1), Vector1, 1);
  Vector1 = vsetq_lane_f32(
    vgetq_lane_f32(Vector1, 2) / vgetq_lane_f32(Vector2, 2), Vector1, 2);
  Vector1 = vsetq_lane_f32(
    vgetq_lane_f32(Vector1, 3) / vgetq_lane_f32(Vector2, 3), Vector1, 3);
  return Vector1;
#elif defined(MLAS_SSE2_INTRINSICS)
...
inline MLAS_FLOAT32X4 MlasGreaterThanFloat32x4(MLAS_FLOAT32X4 Vector1,
                                               MLAS_FLOAT32X4 Vector2) {
#if defined(MLAS_NEON_INTRINSICS)
...
inline MLAS_FLOAT32X4 MlasAndFloat32x4(MLAS_FLOAT32X4 Vector1,
                                       MLAS_FLOAT32X4 Vector2) {
#if defined(MLAS_SSE2_INTRINSICS)
...
inline MLAS_FLOAT32X4 MlasOrFloat32x4(MLAS_FLOAT32X4 Vector1,
                                      MLAS_FLOAT32X4 Vector2) {
#if defined(MLAS_SSE2_INTRINSICS)
...
inline MLAS_FLOAT32X4 MlasAndNotFloat32x4(MLAS_FLOAT32X4 VectorNot,
...
inline MLAS_FLOAT32X4 MlasXorFloat32x4(MLAS_FLOAT32X4 Vector1,
                                       MLAS_FLOAT32X4 Vector2) {
#if defined(MLAS_SSE2_INTRINSICS)
...
inline MLAS_FLOAT32X4 MlasBlendFloat32x4(MLAS_FLOAT32X4 Vector1,
                                         MLAS_FLOAT32X4 Vector2,
...
inline MLAS_FLOAT32X4 MlasMaximumFloat32x4(MLAS_FLOAT32X4 Vector1,
                                           MLAS_FLOAT32X4 Vector2) {
#if defined(MLAS_NEON_INTRINSICS)
...
inline MLAS_FLOAT32X4 MlasMinimumFloat32x4(MLAS_FLOAT32X4 Vector1,
                                           MLAS_FLOAT32X4 Vector2) {
#if defined(MLAS_NEON_INTRINSICS)
...
inline MLAS_FLOAT32X4 MlasClampFloat32x4(MLAS_FLOAT32X4 Value, float LowerRange,
                                         float UpperRange) {
#if defined(MLAS_SSE2_INTRINSICS)
...
inline float MlasReduceAddFloat32x4(MLAS_FLOAT32X4 Vector) {
#if defined(MLAS_NEON64_INTRINSICS)
...
inline float MlasReduceMaximumFloat32x4(MLAS_FLOAT32X4 Vector) {
#if defined(MLAS_NEON64_INTRINSICS)
...
#else
  Vector =
    MlasMaximumFloat32x4(Vector, MlasShuffleFloat32x4<2, 3, 2, 3>(Vector));
  Vector =
    MlasMaximumFloat32x4(Vector, MlasShuffleFloat32x4<1, 1, 1, 1>(Vector));
  return MlasExtractLaneFloat32x4<0>(Vector);
#endif
}

inline float MlasReduceMinimumFloat32x4(MLAS_FLOAT32X4 Vector) {
#if defined(MLAS_NEON64_INTRINSICS)
...
#else
  Vector =
    MlasMinimumFloat32x4(Vector, MlasShuffleFloat32x4<2, 3, 2, 3>(Vector));
  Vector =
    MlasMinimumFloat32x4(Vector, MlasShuffleFloat32x4<1, 1, 1, 1>(Vector));
  return MlasExtractLaneFloat32x4<0>(Vector);
#endif
}

// calc 2^int(N)
inline MLAS_FLOAT32X4 MlasPowerOf2Float32x4(MLAS_FLOAT32X4 Vector) {
  MLAS_INT32X4 emm0 =
...
template <unsigned Lane>
inline double MlasExtractLaneFloat64x2(MLAS_FLOAT64X2 Vector) {
  return Vector[Lane];
}
inline MLAS_FLOAT64X2 MlasMultiplyAddFloat64x2(MLAS_FLOAT64X2 Vector1,
                                               MLAS_FLOAT64X2 Vector2,
                                               MLAS_FLOAT64X2 Vector3) {
  return vec_madd(Vector1, Vector2, Vector3);
}

inline MLAS_FLOAT64X2 MlasBroadcastFloat64x2(const double *Value) {
...
#endif
inline MLAS_FLOAT64X2 MlasBroadcastFloat64x2(double Value) {
#if defined(MLAS_SSE2_INTRINSICS)
...
inline MLAS_FLOAT64X2 MlasZeroFloat64x2(void) {
#if defined(MLAS_SSE2_INTRINSICS)
...
inline MLAS_FLOAT64X2 MlasLoadFloat64x2(const double *Buffer) {
#if defined(MLAS_SSE2_INTRINSICS)
...
inline void MlasStoreFloat64x2(double *Buffer, MLAS_FLOAT64X2 Vector) {
#if defined(MLAS_SSE2_INTRINSICS)
...
inline void MlasStoreAlignedFloat64x2(double *Buffer, MLAS_FLOAT64X2 Vector) {
#if defined(MLAS_SSE2_INTRINSICS)
...
inline MLAS_FLOAT64X2 MlasMultiplyFloat64x2(MLAS_FLOAT64X2 Vector1,
                                            MLAS_FLOAT64X2 Vector2) {
#if defined(MLAS_SSE2_INTRINSICS)
...
inline uint64_t MlasReadTimeStampCounter(void) {
#ifdef _WIN32
#if defined(MLAS_TARGET_AMD64_IX86)
...
inline constexpr size_t UpAlignSize(size_t size) {
  size = (size + ThreadedBufAlignment - 1) / ThreadedBufAlignment;
  return size * ThreadedBufAlignment;
}

inline void MlasThreadedBufAlloc(size_t size) {
...
template <bool Signed> struct Int4Traits;

template <> struct Int4Traits<true> {
  using UnpackedType = int8_t;
  static constexpr int8_t Min = -8;
  static constexpr int8_t Max = 7;
};

template <> struct Int4Traits<false> {
...
inline void MlasSetInt4Element(uint8_t *Output, size_t ElemIndex,
                               UnpackedType Value) {
...
template <typename UnpackedType>
inline void MlasPackInt4Elements(uint8_t *Output, UnpackedType ValueLow,
