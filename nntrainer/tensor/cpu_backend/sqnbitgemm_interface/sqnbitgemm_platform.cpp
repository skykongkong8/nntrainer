...
#define __power_10_andup()                                                     \
  (_system_configuration.implementation & POWER_10_ANDUP)
#endif
#endif
...
MLAS_INTERNAL_DATA MLAS_DECLSPEC_ALIGN(const uint32_t MlasMaskMoveAvx[8],
                                       32) = {0, 1, 2, 3, 4, 5, 6, 7};
...
MLAS_INTERNAL_DATA MLAS_DECLSPEC_ALIGN(const uint32_t MlasMaskMoveLasx[8],
                                       32) = {0, 1, 2, 3, 4, 5, 6, 7};
...
--*/
{
  ...
  // MlasPoolAverageExcludePadFloatKernelSse;
  // this->PoolFloatKernel[MlasAveragePoolingIncludePad] =
  // MlasPoolAverageIncludePadFloatKernelSse; this->ComputeExpF32Kernel =
  // MlasComputeExpF32Kernel; this->LogisticKernelRoutine = MlasLogisticKernel;
  ...
  // this->ComputeLogSoftmaxOutputF32Kernel =
  // MlasComputeLogSoftmaxOutputF32Kernel; this->ReduceMaximumF32Kernel =
  // MlasReduceMaximumF32Kernel; this->ReduceMinimumMaximumF32Kernel =
  // MlasReduceMinimumMaximumF32Kernel; this->QLinearAddS8Kernel =
  // MlasQLinearAddS8Kernel; this->QLinearAddU8Kernel = MlasQLinearAddU8Kernel;
  ...
  if ((Cpuid1[2] & 0x18000000) == 0x18000000) {
    ...
    if ((xcr0 & 0x6) == 0x6) {
      ...
      // this->PoolFloatKernel[MlasMaximumPooling] =
      // MlasPoolMaximumFloatKernelAvx;
      // this->PoolFloatKernel[MlasAveragePoolingExcludePad] =
      // MlasPoolAverageExcludePadFloatKernelAvx;
      // this->PoolFloatKernel[MlasAveragePoolingIncludePad] =
      // MlasPoolAverageIncludePadFloatKernelAvx;
      // this->ComputeSoftmaxOutputF32Kernel =
      // MlasComputeSoftmaxOutputF32KernelAvx;
      // this->ComputeLogSoftmaxOutputF32Kernel =
      // MlasComputeLogSoftmaxOutputF32KernelAvx; this->ReduceMaximumF32Kernel =
      // MlasReduceMaximumF32KernelAvx; this->ReduceMinimumMaximumF32Kernel =
      // MlasReduceMinimumMaximumF32KernelAvx; this->GemmU8U8Kernel = nullptr;
      ...
      if (((Cpuid1[2] & 0x1000) != 0) && ((Cpuid7[1] & 0x20) != 0)) {
        ...
        // this->ConvDepthwiseU8S8Kernel = MlasConvDepthwiseKernelAvx2<uint8_t,
        // int8_t>; this->ConvDepthwiseU8U8Kernel =
        // MlasConvDepthwiseKernelAvx2<uint8_t, uint8_t>;
        // this->ConvDepthwiseS8S8Kernel = MlasConvDepthwiseKernelAvx2<int8_t,
        // int8_t>; this->ConvDepthwiseS8U8Kernel =
        ...
        if (((Cpuid7[1] & 0x10000) != 0) && ((xcr0 & 0xE0) == 0xE0)) {
          ...
          // this->ConvDepthwiseFloatKernel =
          // MlasConvDepthwiseFloatKernelAvx512F; this->ConvPointwiseFloatKernel
          // = MlasConvPointwiseFloatKernelAvx512F;
          // this->PoolFloatKernel[MlasMaximumPooling] =
          // MlasPoolMaximumFloatKernelAvx512F;
          // this->PoolFloatKernel[MlasAveragePoolingExcludePad] =
...
#endif // (defined(_MSC_VER) && (_MSC_VER >= 1933)) || (defined(__GNUC__) &&
       // (__GNUC__ >= 13))
  ...
  HasDotProductInstructions =
    (IsProcessorFeaturePresent(PF_ARM_V82_DP_INSTRUCTIONS_AVAILABLE) != 0);
#else
  // Use the cpuinfo value which is read from sysctl and has some additional
  // special cases.
  // https://github.com/pytorch/cpuinfo/blob/959002f82d7962a473d8bf301845f2af720e0aa4/src/arm/mach/init.c#L369-L379
  // Do NOT use ID_AA64ISAR0_EL1. It causes illegal instruction errors on Mac M1
  // and ARMv8-A chips as well as failing on other ARM chips as it is an EL1
  // level register that requires extra privileges to read.
  //
  // uint64_t isar0_el1;
...
#if defined(__linux__)
  bool HasP10Instructions =
    ((hwcap2 & PPC_FEATURE2_MMA) && (hwcap2 & PPC_FEATURE2_ARCH_3_1));
#elif defined(_AIX)
  bool HasP10Instructions =
    (__power_10_andup() && __power_mma_version() == MMA_V31);
#endif // __linux__
  ...
  if (cap_lasx) {
    ...
    // this->PoolFloatKernel[MlasMaximumPooling] =
    // MlasPoolMaximumFloatKernelLasx;
    // this->PoolFloatKernel[MlasAveragePoolingExcludePad] =
    // MlasPoolAverageExcludePadFloatKernelLasx;
    // this->PoolFloatKernel[MlasAveragePoolingIncludePad] =
    // MlasPoolAverageIncludePadFloatKernelLasx; this->ReduceMaximumF32Kernel =
    // MlasReduceMaximumF32KernelLasx; this->ComputeSoftmaxOutputF32Kernel =
    // MlasComputeSoftmaxOutputF32KernelLasx;
    // this->ComputeLogSoftmaxOutputF32Kernel =
    // MlasComputeLogSoftmaxOutputF32KernelLasx; this->TransposePackB16x4Routine
    // = MlasSgemmTransposePackB16x4Lasx;
    ...
    // this->PoolFloatKernel[MlasMaximumPooling] =
    // MlasPoolMaximumFloatKernelLSX;
    // this->PoolFloatKernel[MlasAveragePoolingExcludePad] =
    // MlasPoolAverageExcludePadFloatKernelLSX;
    // this->PoolFloatKernel[MlasAveragePoolingIncludePad] =
    // MlasPoolAverageIncludePadFloatKernelLSX; this->ReduceMaximumF32Kernel =
    // MlasReduceMaximumF32Kernel; this->ComputeSoftmaxOutputF32Kernel =
    // MlasComputeSoftmaxOutputF32Kernel; this->ComputeLogSoftmaxOutputF32Kernel
...
#ifdef _MSC_VER
thread_local std::unique_ptr<uint8_t, decltype(&_aligned_free)>
  ThreadedBufHolder(nullptr, &_aligned_free);
#else
thread_local std::unique_ptr<uint8_t, decltype(&free)>
  ThreadedBufHolder(nullptr, &free);
#endif
