...
Abstract:
    ...
    MlasGemmQuantOperation and MlasGemmQuantPackedOperation are shared kernel
drivers. MlasGemmQuantScaleSumBuffer is a helper function.
...
inline bool MlasGemmQuantTryGemvKernel(const uint8_t *A, const uint8_t *B,
                                       size_t ldb, int32_t *C, size_t CountK,
                                       size_t CountN, bool AIsSigned,
                                       bool BIsSigned) {
...
template <typename KernelType>
inline constexpr int32_t MlasGemmQuantFixupZeroPointA(int32_t ZeroPointA,
                                                      bool AIsSigned) {
  MLAS_UNREFERENCED_PARAMETER(AIsSigned);
  return ZeroPointA;
}
...
template <typename KernelType>
int32_t constexpr MlasGemmQuantFixupZeroPointB(int32_t ZeroPointB,
                                               bool BIsSigned) {
  MLAS_UNREFERENCED_PARAMETER(BIsSigned);
...
template <typename KernelType>
inline void MlasGemmQuantFixupZeroPointB(const uint8_t *PackedZeroPointB,
                                         int32_t *ZeroPointBBuffer, size_t N,
                                         bool BIsSigned) {
  int32_t ZeroPointB;
  ...
  for (size_t n = 0; n < N; n++) {
    ...
    ZeroPointB =
      MlasGemmQuantFixupZeroPointB<KernelType>(ZeroPointB, BIsSigned);
  ...
  size_t AlignedN = (N + MLAS_QGEMM_STRIDEN_THREAD_ALIGN - 1) &
...
template <typename KernelType>
void MlasGemmQuantCopyPackA(typename KernelType::PackedAType *D,
                            const uint8_t *A, size_t lda, size_t CountM,
                            size_t CountK, int32_t *RowSumBuffer,
                            bool AIsSigned);

template <typename KernelType>
void MlasGemmQuantCopyPackB(typename KernelType::PackedBType *D,
                            const uint8_t *B, size_t ldb, size_t CountN,
                            size_t CountK, int32_t *ColumnSumBuffer,
                            bool BIsSigned);

template <typename KernelType>
size_t MlasGemmQuantKernel(const typename KernelType::PackedAType *A,
                           const typename KernelType::PackedBType *B,
                           int32_t *C, size_t PackedCountK, size_t CountM,
 ...
 */
template <typename KernelType>
size_t MlasSymmQGemmKernel(const int8_t *A, const int8_t *B, int32_t *C,
                           size_t PackedCountK, size_t CountM, size_t CountN,
                           size_t ldc, size_t lda,
                           const int32_t *ColumnSumVector);

inline void MlasGemmQuantScaleSumBuffer(int32_t *Output, const int32_t *Input,
                                        size_t N, int32_t Scale) {
  for (size_t n = 0; n < N; n++) {
    Output[n] = Input[n] * Scale;
  }
}

inline void MlasGemmQuantScaleSumBuffer(int32_t *SumBuffer, size_t N,
                                        int32_t Scale) {
  return MlasGemmQuantScaleSumBuffer(SumBuffer, SumBuffer, N, Scale);
}

template <typename KernelType> inline void MlasGemmQuantThreadInit() {
  ...
  constexpr size_t packBSize = UpAlignSize(
    Strides.N * Strides.K * sizeof(typename KernelType::PackedBType));
  ...
  constexpr size_t packedASize =
    UpAlignSize(PackedStrides.M * PackedStrides.K *
                sizeof(typename KernelType::PackedAType));

  constexpr size_t bufsize = std::max(packASize + packBSize, packedASize) +
                             rowSumSize + colSumSize + zpbSize;

  MlasThreadedBufAlloc(bufsize);
}

template <typename KernelType>
void MlasGemmQuantOperation(const MLAS_GEMM_QUANT_SHAPE_PARAMS *Shape,
                            const MLAS_GEMM_QUANT_DATA_PARAMS *Data,
                            const size_t RangeStartM, const size_t RangeCountM,
                            const size_t RangeStartN, const size_t RangeCountN)
/*++
...
Arguments:
    ...
    Data  - Supplies the structure containing the GEMM input and output data
layout
...
--*/
{
  ...
  if ((RangeCountM == 1) && (ZeroPointA == 0) &&
      (PackedZeroPointB == nullptr) && (ZeroPointB == 0) &&
      (Data->OutputProcessor == nullptr)) {
    if (MlasGemmQuantTryGemvKernel<KernelType>(
          A, B, ldb, C, K, RangeCountN, Shape->AIsSigned, Shape->BIsSigned)) {
      return;
    }
  }
  ...
  ZeroPointA =
    MlasGemmQuantFixupZeroPointA<KernelType>(ZeroPointA, Shape->AIsSigned);
  ...
  ZeroPointB =
    MlasGemmQuantFixupZeroPointB<KernelType>(ZeroPointB, Shape->BIsSigned);
  ...
  for (size_t k = 0; k < K; k += CountK) {
    ...
    const size_t PackedCountK =
      (CountK + KernelType::PackedK - 1) / KernelType::PackedK;
    ...
    for (size_t n = 0; n < RangeCountN; n += CountN) {
      ...
      if (PackedZeroPointB != nullptr) {
        MlasGemmQuantFixupZeroPointB<KernelType>(
          PackedZeroPointB + n, ZeroPointBBuffer, CountN, Shape->BIsSigned);
      }
      ...
      MlasGemmQuantCopyPackB<KernelType>(PanelB, B + n, ldb, CountN, CountK,
                                         ColumnSumBuffer, Shape->BIsSigned);
      ...
      for (size_t m = 0; m < RangeCountM; m += CountM) {
        ...
        MlasGemmQuantCopyPackA<KernelType>(PanelA, A + m * lda, lda, CountM,
                                           CountK, RowSumBuffer,
                                           Shape->AIsSigned);

        //
        // Apply the global depth value constant without the ZeroPointB scaling
        // from:
        //
        ...
        //     * ZeroPointB
        //
        // The ZeroPointB term is factored out and either applied below for
        // per-matrix quantization or inside the kernel for per-column
        // quantization.
        //
        ...
        while (RowsRemaining > 0) {
          ...
          if (PostProcess && Data->OutputProcessor != nullptr) {
            Data->OutputProcessor->Process(
              Data->C, RangeStartM + m + CountM - RowsRemaining,
...
template <typename KernelType>
void MlasGemmQuantPackedOperation(const MLAS_GEMM_QUANT_SHAPE_PARAMS *Shape,
                                  const MLAS_GEMM_QUANT_DATA_PARAMS *Data,
...
--*/
{
  ...
  const size_t AlignedN = (Shape->N + MLAS_QGEMM_STRIDEN_THREAD_ALIGN - 1) &
                          ~(MLAS_QGEMM_STRIDEN_THREAD_ALIGN - 1);
 ...
 */
template <typename KernelType>
void MlasSymmQGemmPackedOperation(const MLAS_GEMM_QUANT_SHAPE_PARAMS *Shape,
  ...
  const size_t PackedCountK =
    (K + KernelType::PackedK - 1) / KernelType::PackedK;
  ...
  while (RowsRemaining > 0) {
    size_t RowsHandled = MlasSymmQGemmKernel<KernelType>(
      pa, b, c, PackedCountK, RowsRemaining, RangeCountN, ldc, lda,
      PackedColumnSumBuffer);
...
typedef void(MLAS_GEMM_QUANT_OPERATION)(
  const MLAS_GEMM_QUANT_SHAPE_PARAMS *Shape,
  const MLAS_GEMM_QUANT_DATA_PARAMS *Data, const size_t RangeStartM,
  const size_t RangeCountM, const size_t RangeStartN, const size_t RangeCountN);

typedef void(MLAS_SYMM_QGEMM_OPERATION)(
  const MLAS_GEMM_QUANT_SHAPE_PARAMS *Shape,
  const MLAS_SYMM_QGEMM_DATA_PARAMS *Data, const size_t RangeStartM,
  const size_t RangeCountM, const size_t RangeStartN, const size_t RangeCountN);

typedef void(MLAS_GEMM_QUANT_COPY_PACKB_ROUTINE)(uint8_t *D, const uint8_t *B,
                                                 size_t ldb, size_t CountN,
                                                 size_t CountK,
...
struct MLAS_SYMM_QGEMM_DISPATCH {
  MLAS_SYMM_QGEMM_OPERATION
    *LitOperation; /// running on little cores with narrow memory load
  MLAS_SYMM_QGEMM_OPERATION
    *BigOperation; /// running on big cores with wider memory load
  MLAS_GEMM_QUANT_COPY_PACKB_ROUTINE *CopyPackBRoutine;
  size_t StrideM; /**< num of rows processed by kernel at a time */
  size_t PackedK;
};

inline const MLAS_GEMM_QUANT_DISPATCH *
MlasGemmQuantGetDispatch(bool AIsSigned, bool BIsSigned) {
  const MLAS_GEMM_QUANT_DISPATCH *GemmQuantDispatch =
    &MlasGemmQuantDispatchDefault;

#if defined(MLAS_TARGET_AMD64_IX86) || defined(MLAS_TARGET_LARCH64)
  if (AIsSigned) {
    GemmQuantDispatch = BIsSigned ? GetMlasPlatform().GemmS8S8Dispatch
                                  : GetMlasPlatform().GemmS8U8Dispatch;
  } else {
    GemmQuantDispatch = BIsSigned ? GetMlasPlatform().GemmU8S8Dispatch
                                  : GetMlasPlatform().GemmU8U8Dispatch;
  }
#elif defined(MLAS_TARGET_ARM64)
  if (BIsSigned) {
    GemmQuantDispatch = AIsSigned ? GetMlasPlatform().GemmS8S8Dispatch
                                  : GetMlasPlatform().GemmU8S8Dispatch;
  } else if (!AIsSigned) {
    GemmQuantDispatch = GetMlasPlatform().GemmU8U8Dispatch;
  }
#elif defined(MLAS_TARGET_ARM64EC) ||                                          \
  (defined(MLAS_TARGET_ARM) && !defined(_MSC_VER))
...
#elif defined(MLAS_TARGET_POWER) && (defined(__linux__) || defined(_AIX)) &&   \
  defined(POWER10) &&                                                          \
  ((defined(__GNUC__) &&                                                       \
  ...
  if (nullptr == GemmQuantDispatch) {
    std::stringstream ss;
    ss << "Quant GEMM format: AIsSigned(" << AIsSigned << "), BIsSigned("
       << BIsSigned << ") is not supported on this device";
