...
template <typename Environment> class ThreadPoolTempl;
...
public:
...
#endif
  // Constructs a pool for running with with "degree_of_parallelism" threads
  // with specified "name". env->StartThread() is used to create individual
  // threads with the given ThreadOptions. If "low_latency_hint" is true the
  // thread pool implementation may use it as a hint that lower latency is
  // preferred at the cost of higher CPU usage, e.g. by letting one or more idle
  // threads spin wait. Conversely, if the threadpool is used to schedule
  // high-latency operations like I/O the hint should be set to false.
  //
  // REQUIRES: degree_of_parallelism > 0
  ThreadPool(Env *env, const ThreadOptions &thread_options,
             const NAME_CHAR_TYPE *name, int degree_of_parallelism,
             bool low_latency_hint, bool force_hybrid = false);
  ...
  // synchronously if it cannot be enqueued.  This will occur if the thread
  // pool's degree-of-parallelism is 1, but it may also occur for
  // implementation-dependent reasons such as if queues used for buffering work
  // are full.
  ...
  static void TryParallelFor(
    ...
    TryParallelFor(tp, total,
                   TensorOpCost{0, 0, static_cast<double>(cost_per_unit)}, fn);
  }
  ...
  inline static void
  TrySimpleParallelFor(ThreadPool *tp, std::ptrdiff_t total,
  ...
  /**
   * Tries to call the given function in parallel, with calls split into
   *(num_batches) batches. \param num_batches If it is zero, it will be replaced
   *to the value of DegreeOfParallelism(). \param fn A std::function or STL
   *style functor with signature of "void f(std::ptrdiff_t);" Pitfall: Caller
   *should cap `num_batches` to a reasonable value based on the cost of `fn` and
   *the value of `total`. For example, if fn is as simple as: int sum=0; fn =
   *[&](int i){sum +=i;} and `total` is 100, then num_batches should be just 1.
   *
   * ```
   **/
  template <typename F>
  inline static void TryBatchParallelFor(ThreadPool *tp, std::ptrdiff_t total,
                                         F &&fn, std::ptrdiff_t num_batches) {
    if (tp == nullptr) {
      for (std::ptrdiff_t i = 0; i < total; ++i) {
        // In many cases, fn can be inlined here.
        fn(i);
  ...
  */
  constexpr static WorkInfo PartitionWork(std::ptrdiff_t batch_idx,
  ...
  // Return the degree of parallelism that code should assume when using the
  // thread pool. It decouples the degree of parallelism for use with the thread
  // pool from the implementation choice of whether this matches the number of
  // threads created in the pool.
  //
  // Currently, a loop with degree-of-parallelism N is supported by a pool of
  // N-1 threads working in combination with the thread initiating the loop.
  static int DegreeOfParallelism(const ThreadPool *tp);

  ORT_DISALLOW_COPY_AND_ASSIGNMENT(ThreadPool);

  // StartProfiling and StopProfiling are not to be consumed as public-facing
  // API
...
private:
  ...
  // Returns the number of threads created in the pool.  This may be different
  // from the value returned by DegreeOfParallelism to code using the pool.
  ...
  // Internal (non-static) parallel loop methods.  Unlike the public static
  // methods, these will not handle the cases of OpenMP builds. or builds
  // without a threadpool.
  void ParallelFor(
    std::ptrdiff_t total, double cost_per_unit,
    const std::function<void(std::ptrdiff_t first, std::ptrdiff_t last)> &fn);

  void ParallelFor(
    std::ptrdiff_t total, const TensorOpCost &cost_per_unit,
    const std::function<void(std::ptrdiff_t first, std::ptrdiff_t)> &fn);

  void SimpleParallelFor(std::ptrdiff_t total,
                         const std::function<void(std::ptrdiff_t)> &fn);
  ...
  // If a thread pool is created with degree_of_parallelism != 1 then an
  // underlying EigenThreadPool is used to create OS threads and handle work
  // distribution to them. If degree_of_parallelism == 1 then
  // underlying_threadpool_ is left as nullptr and parallel work is run directly
  // by the caller.
  ExtendedThreadPoolInterface *underlying_threadpool_ = nullptr;

  // If used, underlying_threadpool_ is instantiated and owned by the
  // ThreadPool.
