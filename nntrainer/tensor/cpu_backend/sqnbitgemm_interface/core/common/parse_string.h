...
bool TryParseStringWithClassicLocale(std::string_view str, T &value) {
  ...
  const bool parse_successful =
    is >> parsed_value &&
    is.get() ==
      std::istringstream::traits_type::eof(); // don't allow trailing characters
  if (!parse_successful) {
    return false;
  }
...
inline bool TryParseStringWithClassicLocale(std::string_view str,
                                            std::string &value) {
  value = str;
  return true;
}
...
Status ParseStringWithClassicLocale(std::string_view s, T &value) {
  ORT_RETURN_IF_NOT(TryParseStringWithClassicLocale(s, value),
 ...
 * Parses a value from an entire string.
 */
template <typename T> T ParseStringWithClassicLocale(std::string_view s) {
  T value{};
