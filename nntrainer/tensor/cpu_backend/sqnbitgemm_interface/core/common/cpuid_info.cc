...
// The following function is declared in "core/common/cpuid_uarch.h" but we
// cannot include the whole header file because
...
static inline void GetCPUID(int function_id, int sub_leaf,
                            int data[4]) { // NOLINT
#if defined(_MSC_VER)
...
void CPUIDInfo::X86Init() {
  ...
  if (num_IDs >= 1) {
    ...
    if (data[2] & (1 << 27)) {
      ...
      has_avx_ = has_sse2 && has_ssse3 && (data[2] & (1 << 28)) &&
                 ((value & AVX_MASK) == AVX_MASK);
      ...
      if (num_IDs >= 7) {
        ...
        // Add check for AVX512 Skylake since tensorization GEMM need intrinsics
        // from avx512bw/avx512dq. avx512_skylake = avx512f | avx512vl |
        // avx512cd | avx512bw | avx512dq
        has_avx512_skylake_ =
          has_avx512 && (data[1] & ((1 << 16) | (1 << 17) | (1 << 28) |
                                    (1 << 30) | (1 << 31)));
...
void CPUIDInfo::ArmLinuxInit() {
  ...
  if (pytorch_cpuinfo_init_) {
    ...
    for (uint32_t c = 0; c < core_cnt; c++) {
      ...
      if (uarch == cpuinfo_uarch_cortex_a53 ||
...
// ARM32 certainly doesn't have fp16, so we will skip the logic to avoid using
// RegGetValueA Windows API
#if !defined(_M_ARM)
#pragma region Application Family or OneCore Family
  ...
  for (int i = 0; i < MAX_CORES - 1; i++) {
    snprintf(midrKey, MAX_VALUE_NAME,
     ...
     *     ((op2 & 7) << 0))
     *
     * For the CP value of MIDR, op0 = 3 and the others are all = 0, so we come
     * up with 0x4000,
     */
    auto retCode =
      ::RegGetValueA(HKEY_LOCAL_MACHINE, midrKey, "CP 4000", RRF_RT_REG_QWORD,
                     nullptr, &midrVal, &midrSize);
...
void CPUIDInfo::ArmAppleInit() {
  ...
  if (pytorch_cpuinfo_init_) {
    ...
    // Note: We leave is_armv8_narrow_ld_ unset because it only applies to a
    // limited set of uarchs that we don't expect to encounter on Apple
    // platforms.
...
CPUIDInfo::CPUIDInfo() {
  ...
  if (!pytorch_cpuinfo_init_) {
    LOGS_DEFAULT(WARNING) << "Failed to initialize PyTorch cpuinfo library. "
                             "May cause CPU EP performance degradation "
