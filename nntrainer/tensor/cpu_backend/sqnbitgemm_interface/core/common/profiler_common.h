...
struct EventRecord {
  ...
  EventRecord(EventCategory category, int process_id, int thread_id,
              const std::string &event_name, long long time_stamp,
              long long duration,
...
//   called when profiling starts virtual void EndProfiling(TimePoint
//   start_time, Events& events) = 0;  // called when profiling ends, save all
//   captures numbers to "events" virtual void Start(uint64_t){}; // called
//   before op start, accept an id as argument to identify the op virtual void
//   Stop(uint64_t){};                                        // called after op
//   stop, accept an id as argument to identify the op
// };
