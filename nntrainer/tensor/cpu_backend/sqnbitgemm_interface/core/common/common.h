...
#define ORT_NOT_IMPLEMENTED(...)                                               \
  do {                                                                         \
    ::onnxruntime::PrintFinalMessage(::onnxruntime::NotImplementedException(   \
...
// inline long long TimeDiffMicroSeconds(TimePoint start_time, TimePoint
// end_time) {
//   return std::chrono::duration_cast<std::chrono::microseconds>(end_time -
//   start_time).count();
// }
...
template <typename Key, typename... OtherContainerArgs,
          ...
          typename LookupKey>
inline bool
Contains(const AssociativeContainer<Key, OtherContainerArgs...> &container,
         LookupKey &&key) {
