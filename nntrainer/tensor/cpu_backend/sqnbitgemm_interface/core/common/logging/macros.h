...
#define LOGS_CATEGORY(logger, severity, category)                              \
  if (!(logger).OutputIsEnabled(::onnxruntime::logging::Severity::k##severity, \
    ...
    CREATE_MESSAGE(logger, severity, category,                                 \
                   ::onnxruntime::logging::DataType::SYSTEM)                   \
      .Stream()
...
#define LOGF_CATEGORY(logger, severity, category, format_str, ...)             \
  do {                                                                         \
    if ((logger).OutputIsEnabled(                                              \
          ::onnxruntime::logging::Severity::k##severity,                       \
          ::onnxruntime::logging::DataType::SYSTEM))                           \
      CREATE_MESSAGE(logger, severity, category,                               \
                     ::onnxruntime::logging::DataType::SYSTEM)                 \
...
#define LOGS_DEFAULT_CATEGORY(severity, category)                              \
  LOGS_CATEGORY(::onnxruntime::logging::LoggingManager::DefaultLogger(),       \
                severity, category)

#define LOGS_USER_DEFAULT_CATEGORY(severity, category)                         \
  LOGS_USER_CATEGORY(::onnxruntime::logging::LoggingManager::DefaultLogger(),  \
                     severity, category)

#define LOGF_DEFAULT_CATEGORY(severity, category, format_str, ...)             \
  LOGF_CATEGORY(::onnxruntime::logging::LoggingManager::DefaultLogger(),       \
                severity, category, format_str, ##__VA_ARGS__)

#define LOGF_USER_DEFAULT_CATEGORY(severity, category, format_str, ...)        \
  LOGF_USER_CATEGORY(::onnxruntime::logging::LoggingManager::DefaultLogger(),  \
                     severity, category, format_str, ##__VA_ARGS__)
...
#define LOGS_USER_DEFAULT(severity)                                            \
  LOGS_USER_DEFAULT_CATEGORY(severity,                                         \
                             ::onnxruntime::logging::Category::onnxruntime)

#define LOGF_DEFAULT(severity, format_str, ...)                                \
  LOGF_DEFAULT_CATEGORY(severity,                                              \
                        ::onnxruntime::logging::Category::onnxruntime,         \
                        format_str, ##__VA_ARGS__)

#define LOGF_USER_DEFAULT(severity, format_str, ...)                           \
  LOGF_USER_DEFAULT_CATEGORY(severity,                                         \
                             ::onnxruntime::logging::Category::onnxruntime,    \
                             format_str, ##__VA_ARGS__)
...
#define LOGF_CATEGORY_IF(boolean_expression, logger, severity, category,       \
                         format_str, ...)                                      \
  do {                                                                         \
    if ((boolean_expression) == true)                                          \
...
#define LOGF_DEFAULT_CATEGORY_IF(boolean_expression, severity, category,       \
                                 format_str, ...)                              \
  do {                                                                         \
    if ((boolean_expression) == true)                                          \
...
#define LOGF_USER_CATEGORY_IF(boolean_expression, logger, severity, category,  \
                              format_str, ...)                                 \
  do {                                                                         \
    if ((boolean_expression) == true)                                          \
      LOGF_USER_CATEGORY(logger, severity, category, format_str,               \
                         ##__VA_ARGS__);                                       \
  } while (0)

#define LOGF_USER_DEFAULT_CATEGORY_IF(boolean_expression, severity, category,  \
                                      format_str, ...)                         \
  do {                                                                         \
    if ((boolean_expression) == true)                                          \
      LOGF_USER_DEFAULT_CATEGORY(severity, category, format_str,               \
                                 ##__VA_ARGS__);                               \
  } while (0)
...
#define VLOGS(logger, level)                                                   \
  if (!(::onnxruntime::logging::vlog_enabled &&                                \
        level <= (logger).VLOGMaxLevel())) {                                   \
    /* do nothing */                                                           \
  } else                                                                       \
...
#define VLOGF(logger, level, format_str, ...)                                  \
  do {                                                                         \
    if (::onnxruntime::logging::vlog_enabled &&                                \
        level <= (logger).VLOGMaxLevel())                                      \
      LOGF_CATEGORY(logger, VERBOSE, "VLOG" #level, format_str,                \
                    ##__VA_ARGS__);                                            \
  } while (0)
...
#define VLOGF_USER(logger, level, format_str, ...)                             \
  do {                                                                         \
    if (::onnxruntime::logging::vlog_enabled &&                                \
        level <= (logger).VLOGMaxLevel())                                      \
      LOGF_USER_CATEGORY(logger, VERBOSE, "VLOG" #level, format_str,           \
                         ##__VA_ARGS__);                                       \
  } while (0)
#else
// Disabled in Release builds.
...
#define VLOGF_DEFAULT(level, format_str, ...)                                  \
  VLOGF(::onnxruntime::logging::LoggingManager::DefaultLogger(), level,        \
        format_str, ##__VA_ARGS__)

#define VLOGF_USER_DEFAULT(level, format_str, ...)                             \
  VLOGF_USER(::onnxruntime::logging::LoggingManager::DefaultLogger(), level,   \
             format_str, ##__VA_ARGS__)
