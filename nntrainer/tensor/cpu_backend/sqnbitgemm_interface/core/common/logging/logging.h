...
// C++20 has operator<< in std::chrono for Timestamp type but mac builds need
// additional checks to ensure usage is valid.
// TODO: As we enable C++20 on other platforms we may need similar checks.
// define a temporary value to determine whether to use the std::chrono or date
// implementation.
...
// Catalyst check must be first as it has both TARGET_OS_MACCATALYST and
// TARGET_OS_MAC set
#if TARGET_OS_MACCATALYST
// maccatalyst requires version 16.3
#if (defined(__IPHONE_OS_VERSION_MIN_REQUIRED) &&                              \
     __IPHONE_OS_VERSION_MIN_REQUIRED < 160300)
...
#ifndef NDEBUG
ORT_ATTRIBUTE_UNUSED static bool vlog_enabled =
...
enum class ORTTraceLoggingKeyword : uint64_t {
  ...
  Logs = 0x2, // LOGS() Macro ORT logs. Pair with an appropriate level depending
              // on detail required
  Reserved1 = 0x4, // Reserved if we want to add some specific sub-categories
                   // instead of just LOGS() or other uses
  Reserved2 = 0x8,
  Reserved3 = 0x10,
  Reserved4 = 0x20,
  Reserved5 = 0x40,
  Reserved6 = 0x80,
  Profiling = 0x100 // Enables profiling. At higher levels >5 can impact
...
public:
  ...
  */
  bool AddSinkOfType(SinkType sinkType,
                     std::function<std::unique_ptr<ISink>()> sinkFactory,
  ...
  */
  ...
  /**
     Change the maximum verbosity level for log messages to be output by the
     default logger.
     @remarks
     To activate the verbose log, the logger severity must also be set to
     kVERBOSE.
...
public:
  ...
  */
  ...
  /**
     Return the maximum VLOG level allowed. Disabled unless logging VLOG
     messages
  */
...
/**
   If the ONNXRuntimeTraceLoggingProvider ETW Provider is enabled, then adds to
   the existing logger.
*/
...
/**
  If the ONNXRuntimeTraceLoggingProvider ETW Provider is enabled, then can
  override the logging level. But this overrided level only applies to the ETW
  sink. The original logger(s) retain their original logging level
*/
