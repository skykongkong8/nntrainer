...
// #include "core/common/my_narrow.h"
// #endif

#include "../common/common.h"

#include "../session/onnxruntime_float16.h"
#include "my_narrow.h"
...
struct BFloat16 : onnxruntime_float16::BFloat16Impl<BFloat16> {
  ...
  ORT_HOST_DEVICE static bool
  AreZeroHostDevice(const BFloat16Impl &lhs, const BFloat16Impl &rhs) noexcept {
...
inline MLFloat16 operator"" _f16(unsigned long long int v) noexcept {
  return MLFloat16::FromBits(my_narrow<uint16_t>(v));
}
...
inline BFloat16 operator"" _b16(unsigned long long int v) noexcept {
  return BFloat16::FromBits((my_narrow<uint16_t>(v)));
}
...
inline void BFloat16ToFloat(const BFloat16 *blf, float *flt,
...
template <> class numeric_limits<onnxruntime::MLFloat16> {
public:
  ...
  static constexpr onnxruntime::MLFloat16 max() noexcept {
    return onnxruntime::MLFloat16::FromBits(
      0x7BFFU); // Largest representable value: 65504
  }

  static constexpr onnxruntime::MLFloat16 lowest() noexcept {
    return onnxruntime::MLFloat16::FromBits(
      0xFBFFU); // Smallest representable value: -65504
  }
  ...
  static constexpr onnxruntime::MLFloat16 quiet_NaN() noexcept {
    // The most significant fraction bit shall be 1, and no limitation on other
    // fraction bits. Note that most frameworks use 0x7E00; while CUDA uses
    // 0x7FFF; .Net System.Half.NaN uses 0xFE00;
    return onnxruntime::MLFloat16::FromBits(
  ...
  static constexpr onnxruntime::MLFloat16 epsilon() noexcept {
    return onnxruntime::MLFloat16::FromBits(
      0x1400U); // Difference between 1.0 and the next value: 2^-10 =
                // 0.0009765625
  }
  ...
  static constexpr int max_digits10 =
    5; // Max decimal digits required for precision
...
template <> class numeric_limits<onnxruntime::BFloat16> {
public:
  ...
  static constexpr onnxruntime::BFloat16 quiet_NaN() noexcept {
    // The most significant fraction bit shall be 1, and no limitation on other
    // fraction bits. Note that Torch, Tensorflow, OpenVino, nGraph uses 0x7FC0;
    // Paddle uses 0x7FC1; CUDA uses 0x7FFF.
    return onnxruntime::BFloat16::FromBits(
      0x7FC1U); // Bits: sign(0), exponent(111,1111,1), fraction(100,0001)
  }

  static constexpr onnxruntime::BFloat16 signaling_NaN() noexcept {
    // The most significant fraction bit shall be 0, and there is at least one 1
    // in other fraction bits.
    return onnxruntime::BFloat16::FromBits(
  ...
  static constexpr onnxruntime::BFloat16 denorm_min() noexcept {
    return onnxruntime::BFloat16::FromBits(
      0x0001U); // Minimum subnormal value: 9.1835e-41
  }
