...
void MlasGemmQuantThreaded(const MLAS_GEMM_QUANT_WORK_BLOCK *WorkBlock,
...
Arguments:
    ...
    ThreadInfo - Supplies the structure containing the thread task partition
info.
...
--*/
{
  ...
  MlasPartitionWork(ThreadIdM, WorkBlock->ThreadCountM, M, &RangeStartM,
                    &RangeCountM);
  ...
  MlasPartitionWork(ThreadIdN, WorkBlock->ThreadCountN, BlockedN, &RangeStartN,
                    &RangeCountN);
  ...
  const auto *GemmQuantDispatch =
    MlasGemmQuantGetDispatch(Shape->AIsSigned, Shape->BIsSigned);
  ...
  GemmQuantOperation(Shape, Data, RangeStartM, RangeCountM, RangeStartN,
                     RangeCountN);
}
...
void MLASCALL MlasGemmBatch(const MLAS_GEMM_QUANT_SHAPE_PARAMS &Shape,
  ...
  if (Complexity < double(MLAS_QGEMM_THREAD_COMPLEXITY *
...
int32_t MlasSymmQgemmGetKernelOutputCnt() {
  const MLAS_SYMM_QGEMM_DISPATCH *dispatch =
    GetMlasPlatform().SymmQgemmDispatch;
  return int32_t(dispatch->StrideM);
}
...
void MLASCALL MlasSymmQgemmBatch(const MLAS_GEMM_QUANT_SHAPE_PARAMS &Shape,
  ...
  if ((size_t)MlasGetMaximumThreadCount(ThreadPool) > BatchN) {
    ...
    if (max_nc < nc) {
      nc = std::min(
        nc, MlasDivRoundup(nc, max_nc * MLAS_QGEMM_STRIDEN_THREAD_ALIGN) *
              MLAS_QGEMM_STRIDEN_THREAD_ALIGN);
    }
  }
  ...
  MlasTrySimpleParallel(
    ThreadPool, ThreadsPerGemm * BatchN, [&](ptrdiff_t tid) {
      ...
      const size_t RangeCountM =
        std::min(Shape.M - RangeStartM, (size_t)StrideM);

      const size_t RangeStartN = ThreadIdN * StrideN;
      const size_t RangeCountN =
        std::min(Shape.N - RangeStartN, (size_t)StrideN);

      operation(&Shape, Data, RangeStartM, RangeCountM, RangeStartN,
                RangeCountN);
    });
}
...
--*/
{
  ...
  const auto *GemmQuantDispatch =
    MlasGemmQuantGetDispatch(AIsSigned, BIsSigned);
  ...
  const size_t AlignedN = (N + MLAS_QGEMM_STRIDEN_THREAD_ALIGN - 1) &
...
--*/
{
  ...
  for (size_t k = 0; k < K; k += CountK) {
    ...
    for (size_t n = 0; n < N; n += CountN) {
      ...
      GemmQuantDispatch->CopyPackBRoutine(pb, B + n, ldb, CountN, CountK,
                                          ColumnSumBuffer, BIsSigned);
...
void MLASCALL MlasSymmQgemmPackB(size_t N, size_t K, const int8_t *B,
  ...
  const MLAS_SYMM_QGEMM_DISPATCH *SymmQgemmDispatch =
    GetMlasPlatform().SymmQgemmDispatch;
  ...
  SymmQgemmDispatch->CopyPackBRoutine((uint8_t *)PackedB, (const uint8_t *)B,
                                      ldb, N, K, PackedColumnSumBuffer, true);
