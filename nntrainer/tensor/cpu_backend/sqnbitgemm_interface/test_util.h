...
template <typename T> class MatrixGuardBuffer {
public:
  MatrixGuardBuffer() {
    _BaseBuffer = nullptr;
    _BaseBufferSize = 0;
  ...
  T *GetFilledBuffer(size_t Elements,
                     std::function<void(T *, size_t)> const &fillFunc) {
    //
    ...
    if (Elements > _ElementsAllocated) {
      ...
      size_t BytesToAllocate =
        ((Elements * sizeof(T)) + BufferAlignment - 1) & ~(BufferAlignment - 1);

      _BaseBufferSize = BytesToAllocate + GuardPadding;

#if defined(_WIN32)
      _BaseBuffer =
        VirtualAlloc(NULL, _BaseBufferSize, MEM_RESERVE, PAGE_NOACCESS);
#else
      _BaseBuffer =
        mmap(0, _BaseBufferSize, PROT_NONE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
#endif
...
#if defined(_WIN32)
      if (VirtualAlloc(_BaseBuffer, BytesToAllocate, MEM_COMMIT,
  ...
  T *GetBuffer(size_t Elements, bool ZeroFill = false) {
    if (ZeroFill) {
      return GetFilledBuffer(Elements, [](T *start, size_t size) {
...
//
template <typename TMlasTester> class MlasTestFixture : public testing::Test {
public:
  static void SetUpTestSuite() { mlas_tester = new TMlasTester(); };
...
// Long Execute test. It is too heavy to register each single test, treat long
// execute big groups.
...
public:
  ...
  static size_t RegisterLongExecute() {
    testing::RegisterTest(
      TMlasTester::GetTestSuiteName(), "LongExecute", nullptr, "LongExecute",
      __FILE__, __LINE__,
...
// because they finish quickly, and may disturb others by inject too many small
// tests. Register it as whole using following helper.
template <typename TMlasTester>
...
public:
  ...
  static size_t RegisterShortExecute() {
    testing::RegisterTest(
      TMlasTester::GetTestSuiteName(), "ShortExecute", nullptr, nullptr,
      __FILE__, __LINE__,
      // Important to use the fixture type as the return type here.
      [=]() -> MlasTestFixture<TMlasTester> * {
...
inline void ReorderInputNchw(const int64_t *input_shape, const float *S,
                             float *D) {
  ...
  int64_t nchwc_channel_count =
    (channel_count + nchwc_block_size - 1) & ~(nchwc_block_size - 1);
  int64_t spatial_count = input_shape[2] * input_shape[3];
  for (int64_t n = 0; n < batch_count; n++) {
    MlasReorderInputNchw(S, D, static_cast<size_t>(channel_count),
                         static_cast<size_t>(spatial_count));
