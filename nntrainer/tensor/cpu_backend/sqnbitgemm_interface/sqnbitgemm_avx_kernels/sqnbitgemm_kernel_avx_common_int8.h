/**
 * @file sqnbitgemm_kernel_avx_common_int8.h
...
inline void ComputeDotProducts_BlkBitWidth4_CompInt8_SubBlkLen16(
  size_t BlkLen, const std::byte *QuantARowPtr,
  const std::byte *QuantBDataColPtr, const float *QuantBScaleColPtr,
  const std::byte *QuantBZeroPointColPtr, float *SumPtr, size_t CountK,
  size_t StrideQuantBData, size_t StrideQuantBScale,
  size_t StrideQuantBZeroPoint, const float *BiasPtr) {
  ...
  constexpr size_t SubBlkStep =
    MlasQNBitBlkDataSizeInBytes(BlkBitWidth, SubBlkLen);
  ...
  for (size_t k = 0; k < CountK; k += BlkLen) {
    ...
    float scale_v[NCols];
    UnrolledLoop<NCols>(
      [&](size_t i) { scale_v[i] = (*(s + StrideQuantBScale * i)) * a_scale; });

    std::byte *bptr[NCols];
    UnrolledLoop<NCols>(
      [&](size_t i) { bptr[i] = (std::byte *)(b + StrideQuantBData * i); });
    ...
    UnrolledLoop<NCols>([&](size_t i) {
      ...
      const __m128i upper =
        _mm_bslli_si128(_mm_and_si128(_mm_srli_epi16(bvi[i], 4), low_mask), 8);
    ...
    if constexpr (HasZeroPoint) {
      UnrolledLoop<NCols>([&](size_t i) {
        bv_epi16[i] =
          _mm256_sub_epi16(bv_epi16[i], _mm256_set1_epi16(offset[i]));
      });
    } else {
      const __m256i eight = _mm256_set1_epi16(8);
      UnrolledLoop<NCols>(
        [&](size_t i) { bv_epi16[i] = _mm256_sub_epi16(bv_epi16[i], eight); });
    }
...
void SQ4BitGemmM1Kernel_BlkLen16_CompInt8_Impl(
  ...
  while (nblk >= 0) {
    ComputeDotProducts_BlkBitWidth4_CompInt8_SubBlkLen16<NCols4, HasZeroPoint>(
      BlkLen16, QuantARowPtr, QuantBDataColPtr, QuantBScaleColPtr,
...
void SQ4BitGemmM1Kernel_BlkLen32_CompInt8_Impl(
...
using DotQuadFunctionType = __m256 (*)(const __m256i, const __m256i,
                                       const __m256i, const __m256i);

template <bool HasZeroPoint, DotQuadFunctionType dot_quad>
inline void ComputeDotProducts_BlkBitWidth4_CompInt8_SubBlkLen64_NCols4(
  size_t BlkLen, const std::byte *QuantARowPtr,
  const std::byte *QuantBDataColPtr, const float *QuantBScaleColPtr,
  const std::byte *QuantBZeroPointColPtr, float *SumPtr, size_t CountK,
  size_t StrideQuantBData, size_t StrideQuantBScale,
  ...
  constexpr size_t BlkBitWidth = 4;
  constexpr size_t SubBlkStep =
    MlasQNBitBlkDataSizeInBytes(BlkBitWidth, SubBlkLen64);
  ...
  for (size_t k = 0; k < CountK; k += BlkLen) {
    ...
    if constexpr (HasZeroPoint) {
      ...
      std::byte zp_packed = QuantBZeroPointColPtr[0 * StrideQuantBZeroPoint +
                                                  QuantBZeroPointIdx / 2];
      zp0 = std::to_integer<int8_t>(is_lower ? (zp_packed & std::byte{0x0F})
                                             : (zp_packed >> 4));
      zp_packed = QuantBZeroPointColPtr[1 * StrideQuantBZeroPoint +
                                        QuantBZeroPointIdx / 2];
      zp1 = std::to_integer<int8_t>(is_lower ? (zp_packed & std::byte{0x0F})
                                             : (zp_packed >> 4));
      zp_packed = QuantBZeroPointColPtr[2 * StrideQuantBZeroPoint +
                                        QuantBZeroPointIdx / 2];
      zp2 = std::to_integer<int8_t>(is_lower ? (zp_packed & std::byte{0x0F})
                                             : (zp_packed >> 4));
      zp_packed = QuantBZeroPointColPtr[3 * StrideQuantBZeroPoint +
                                        QuantBZeroPointIdx / 2];
      zp3 = std::to_integer<int8_t>(is_lower ? (zp_packed & std::byte{0x0F})
                                             : (zp_packed >> 4));
    } else {
      zp0 = 8;
      zp1 = 8;
    ...
    for (size_t kk = 0; kk < ck; kk += SubBlkLen64) {
      ...
      __m256i bv1_32_epi8 =
        _mm256_and_si256(_mm256_srli_epi16(bv, 4), low_mask);
      ...
      __m256 sum_ps =
        dot_quad(bv0_32_epi8, bv1_32_epi8, av0_32_epi8, av1_32_epi8);
...
// TODO: is this able to be inlined if DotQuadFunctionType is a function
// pointer?
template <bool HasZeroPoint, DotQuadFunctionType dot_quad>
inline void ComputeDotProducts_BlkBitWidth4_CompInt8_SubBlkLen64_NCols1(
  size_t BlkLen, const std::byte *QuantARowPtr,
  const std::byte *QuantBDataColPtr, const float *QuantBScaleColPtr,
  const std::byte *QuantBZeroPointColPtr, float *SumPtr, size_t CountK,
  size_t StrideQuantBData, size_t StrideQuantBScale,
  ...
  for (size_t k = 0; k < CountK; k += BlkLen) {
    ...
    for (size_t kk = 0; kk < ck; kk += SubBlkLen64) {
      ...
      //__m256 sum_ps = mul_sum_s8_quads_float_avx2(bv_lo_epi8, bv_hi_epi8,
      //a_byte_lo, a_byte_hi);
      acc0 = _mm256_fmadd_ps(_mm256_set1_ps(scale_v0), sum_ps, acc0);
    } // kk
...
template <bool HasZeroPoint, DotQuadFunctionType dot_quad>
void SQ4BitGemmM1Kernel_BlkLen64Plus_CompInt8_Impl(
