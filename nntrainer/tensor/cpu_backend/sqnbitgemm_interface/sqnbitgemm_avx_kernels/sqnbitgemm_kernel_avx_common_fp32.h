/**
 * @file sqnbitgemm_kernel_avx_common_fp32.h
...
inline size_t MlasQ4GemmKernelBlkLen16Avx512f(
  ...
  size_t CountK, size_t BlockCountK, const float *Bias, size_t lda,
  size_t ldc) {
  ...
  for (size_t m = 0; m < CountM; m++) {
    ...
    while (nblk >= 0) {
      ...
      for (size_t k = 0; k < CountK; k += BlkLen16) {
        ...
        const __m128i *b1ptr =
          (const __m128i *)(b_blk_data_ptr + StrideQuantBData * 1);
        const __m128i *b2ptr =
          (const __m128i *)(b_blk_data_ptr + StrideQuantBData * 2);
        const __m128i *b3ptr =
          (const __m128i *)(b_blk_data_ptr + StrideQuantBData * 3);
        ...
        __m128i upper =
          _mm_bslli_si128(_mm_and_si128(_mm_srli_epi16(bvi4_0, 4), lowMask), 8);
        __m128i bytes0 = _mm_add_epi8(upper, lower);

        lower = _mm_and_si128(bvi4_1, lowMask);
        upper =
          _mm_bslli_si128(_mm_and_si128(_mm_srli_epi16(bvi4_1, 4), lowMask), 8);
        __m128i bytes1 = _mm_add_epi8(upper, lower);

        lower = _mm_and_si128(bvi4_2, lowMask);
        upper =
          _mm_bslli_si128(_mm_and_si128(_mm_srli_epi16(bvi4_2, 4), lowMask), 8);
        __m128i bytes2 = _mm_add_epi8(upper, lower);

        lower = _mm_and_si128(bvi4_3, lowMask);
        upper =
          _mm_bslli_si128(_mm_and_si128(_mm_srli_epi16(bvi4_3, 4), lowMask), 8);
        ...
        if constexpr (HasZeroPoint) {
          ...
          std::byte zp_packed =
            QuantBZeroPointColPtr[0 * StrideQuantBZeroPoint +
          ...
          zp_packed = QuantBZeroPointColPtr[1 * StrideQuantBZeroPoint +
                                            QuantBZeroPointIdx / 2];
          zp = std::to_integer<int8_t>(is_lower ? (zp_packed & std::byte{0x0F})
                                                : (zp_packed >> 4));
    ...
    if (nblk > 0) {
      ...
      for (size_t k = 0; k < CountK; k += BlkLen16) {
        ...
        for (int64_t nn = 0; nn < nblk; nn++) {
          // Load B col vectors of 16 of 4b
          // SubBlkLen = 16: | v0 v8 | v1 v9 | v2 vA | v3 vB | v4 vC | v5 vD |
          // v6 vE | v7 vF |
          const __m128i bvi4_0 = _mm_loadl_epi64(b_ptr[nn]++);

          // expand 4b into byte array
          __m128i lower = _mm_and_si128(bvi4_0, lowMask);
          __m128i upper = _mm_bslli_si128(
            _mm_and_si128(_mm_srli_epi16(bvi4_0, 4), lowMask), 8);
          ...
          if constexpr (HasZeroPoint) {
            ...
            // TODO: void condition on is_lower
            std::byte zp_packed =
              QuantBZeroPointColPtr[nn * StrideQuantBZeroPoint +
                                    QuantBZeroPointIdx / 2];
            uint8_t zp = std::to_integer<int8_t>(
              is_lower ? (zp_packed & std::byte{0x0F}) : (zp_packed >> 4));
...
inline size_t MlasQ4GemmKernelBlkLen32PlusAvx512f(
      ...
      for (size_t k = 0; k < CountK; k += BlkLen) {
        ...
        for (size_t kk = 0; kk < ck; kk += MLAS_QUANT4_BLK_UNIT32) {
          ...
          if constexpr (IsBlkLen64Layout) {
            ...
            // load 64 weights at once, parse to get v0 - v31 if subblk is even,
            // otherwise get v32 - v63 increment b_data_ptr by 2 *
            // MLAS_QUANT4_BLK_UNIT32 if subblk is odd so that all v0-63 of the
            // pack are processed.
            ...
            const int count_half_4 = 4 * ((kk % (2 * MLAS_QUANT4_BLK_UNIT32)) /
                                          MLAS_QUANT4_BLK_UNIT32);
            bytes0 = _mm256_and_si256(_mm256_srli_epi16(bvi4_0, count_half_4),
                                      lowMask);
            bytes1 = _mm256_and_si256(_mm256_srli_epi16(bvi4_1, count_half_4),
                                      lowMask);
            bytes2 = _mm256_and_si256(_mm256_srli_epi16(bvi4_2, count_half_4),
                                      lowMask);
            bytes3 = _mm256_and_si256(_mm256_srli_epi16(bvi4_3, count_half_4),
                                      lowMask);
      ...
      for (size_t k = 0; k < CountK; k += BlkLen) {
        ...
        for (size_t kk = 0; kk < ck; kk += MLAS_QUANT4_BLK_UNIT32) {
          ...
          for (int64_t nn = 0; nn < nblk; nn++) {
            ...
            if constexpr (IsBlkLen64Layout) {
              // dst: | v0  v32 | v1  v33 | ... | v30 v62 | v31 v63 |
              // load 64 weights at once, parse to get v0 - v31 if subblk is
              // even, otherwise get v32 - v63 increment b_data_ptr by 2 *
              // MLAS_QUANT4_BLK_UNIT32 if subblk is odd so that all v0-63 of
              // the pack are processed.
              const __m256i bvi4 =
                _mm256_loadu_si256((__m256i const *)(b_ptr[nn]));
              const int count_half_4 =
                4 *
                ((kk % (2 * MLAS_QUANT4_BLK_UNIT32)) / MLAS_QUANT4_BLK_UNIT32);
              bytes = _mm256_and_si256(_mm256_srli_epi16(bvi4, count_half_4),
                                       lowMask);
