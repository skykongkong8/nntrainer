...
//
template <typename T> inline float &MlasQ4BlkScale(uint8_t *BlkPtr) {
  return *reinterpret_cast<float *>(BlkPtr);
}

template <typename T> inline float MlasQ4BlkScale(const uint8_t *BlkPtr) {
  return *reinterpret_cast<const float *>(BlkPtr);
}

template <typename T> uint8_t &MlasQ4BlkZeroPoint(uint8_t *BlkPtr);

template <typename T> uint8_t MlasQ4BlkZeroPoint(const uint8_t *BlkPtr);

template <typename T> inline uint8_t *MlasQ4BlkData(uint8_t *BlkPtr) {
  return BlkPtr + sizeof(float);
}

template <typename T>
inline const uint8_t *MlasQ4BlkData(const uint8_t *BlkPtr) {
  return BlkPtr + sizeof(float);
}
...
struct MLAS_Q4TYPE_BLK1 {
  ...
  static constexpr size_t BlobSize =
    BlkLen / 2 + sizeof(float) + sizeof(uint8_t);
};

template <>
inline uint8_t &MlasQ4BlkZeroPoint<MLAS_Q4TYPE_BLK1>(uint8_t *BlkPtr) {
  return *(BlkPtr + sizeof(float));
}

template <>
inline uint8_t MlasQ4BlkZeroPoint<MLAS_Q4TYPE_BLK1>(const uint8_t *BlkPtr) {
  return *(BlkPtr + sizeof(float));
}

template <> inline uint8_t *MlasQ4BlkData<MLAS_Q4TYPE_BLK1>(uint8_t *BlkPtr) {
  return BlkPtr + sizeof(float) + sizeof(uint8_t);
}

template <>
inline const uint8_t *MlasQ4BlkData<MLAS_Q4TYPE_BLK1>(const uint8_t *BlkPtr) {
  return BlkPtr + sizeof(float) + sizeof(uint8_t);
}
