...
inline MLAS_INT32X4 MlasQuantizeLinearVector(MLAS_FLOAT32X4 FloatVector,
...
MLAS_INT32X4 MlasQuantizeLinearPackBytes(MLAS_INT32X4 IntegerVector);

template <typename OutputType>
void MlasQuantizeLinearStore4PackedValues(MLAS_INT32X4 IntegerVector,
                                          OutputType *Output);

template <typename OutputType>
void MlasQuantizeLinearStoreSingleValue(MLAS_INT32X4 IntegerVector,
                                        OutputType *Output);
...
template <>
MLAS_INT32X4 MlasQuantizeLinearPackBytes<uint16_t>(MLAS_INT32X4 IntegerVector) {
  //
...
template <>
MLAS_INT32X4 MlasQuantizeLinearPackBytes<int16_t>(MLAS_INT32X4 IntegerVector) {
  //
  // Swizzle the least significant u16 from each int32_t element to the
  // bottom eight bytes of the vector register.
  //
...
template <typename OutputType>
inline void MlasQuantizeLinearStore4PackedValues(MLAS_INT32X4 IntegerVector,
                                                 OutputType *Output) {
  ...
  if constexpr (std::is_same_v<OutputType, uint8_t> ||
    ...
    static_assert(std::is_same_v<OutputType, uint16_t> ||
                  std::is_same_v<OutputType, int16_t>);
    vst1q_lane_s64(reinterpret_cast<int64_t *>(Output),
                   vreinterpretq_s64_s32(IntegerVector), 0);
  }
}

template <>
inline void
MlasQuantizeLinearStoreSingleValue<uint8_t>(MLAS_INT32X4 IntegerVector,
                                            uint8_t *Output) {
...
template <>
inline void
MlasQuantizeLinearStoreSingleValue<int8_t>(MLAS_INT32X4 IntegerVector,
                                           int8_t *Output) {
  // Copies the lower 8-bit element of the vector into memory (Output).
...
template <>
inline void
MlasQuantizeLinearStoreSingleValue<uint16_t>(MLAS_INT32X4 IntegerVector,
                                             uint16_t *Output) {
...
template <>
inline void
MlasQuantizeLinearStoreSingleValue<int16_t>(MLAS_INT32X4 IntegerVector,
                                            int16_t *Output) {
  // Copies the lower 16-bit element of the vector into memory (Output).
...
template <typename OutputType>
inline void MlasQuantizeLinearStoreSingleValue(MLAS_INT32X4 IntegerVector,
                                               OutputType *Output) {
  ...
  // Expects that the 32-bit element in lane 0 is already within the valid
  // numerical range of the OutputType.
...
MlasQuantizeLinearPackBytes<uint16_t>(MLAS_INT32X4 IntegerVector) {
  ...
  IntegerVector = _mm_packus_epi32(
    IntegerVector, IntegerVector); // 16-bit values packed in lower 8 bytes.
#else
  // Cannot use _mm_packus_epi32 because that was not available until SSE4.1.
  // Instead, emulate by sign-extending the first 16-bits of each packed 32-bit
  // element. Afterwards, can use _mm_packs_epi32, which is available on SSE2.
  // See: https://stackoverflow.com/a/11028244

  IntegerVector = _mm_slli_epi32(IntegerVector, 16);
  IntegerVector = _mm_srai_epi32(
    IntegerVector,
    16); // Sign-extend: undo left shift with right arithmetic shift
  IntegerVector = _mm_packs_epi32(
    IntegerVector, IntegerVector); // 16-bit values packed in lower 8 bytes.
...
inline void MlasQuantizeLinearStore4PackedValues(MLAS_INT32X4 IntegerVector,
  ...
  if constexpr (std::is_same_v<OutputType, uint8_t> ||
    ...
    *(reinterpret_cast<int32_t *>(Output)) = _mm_cvtsi128_si32(IntegerVector);
    ...
    // x86 does not support _mm_cvtsi128_si64, so use _mm_maskmoveu_si128
    // instead.
    constexpr uint32_t bytes_high_bit = 0x80808080;
    const __m128i first_8_bytes_mask =
      _mm_set_epi32(0, 0, bytes_high_bit, bytes_high_bit);
    _mm_maskmoveu_si128(IntegerVector, first_8_bytes_mask,
...
template <typename OutputType>
void MLASCALL MlasQuantizeLinearKernel(const float *Input, OutputType *Output,
...
--*/
{
  ...
  auto MinimumValueVector =
    MlasBroadcastFloat32x4(float(MinimumValue - ZeroPoint));
  auto MaximumValueVector =
    MlasBroadcastFloat32x4(float(MaximumValue - ZeroPoint));
  ...
  while (N >= 4) {
    ...
    auto IntegerVector =
      MlasQuantizeLinearVector(FloatVector, ScaleVector, MinimumValueVector,
...
void MLASCALL MlasQuantizeLinearInt4Kernel(const float *Input, uint8_t *Output,
                                           size_t N, float Scale,
                                           int8_t ZeroPoint) {
  ...
  auto ScaleVector = MlasBroadcastFloat32x4(Scale);
  auto MinimumValueVector =
    MlasBroadcastFloat32x4(static_cast<float>(MinimumValue - ZeroPoint));
  auto MaximumValueVector =
    MlasBroadcastFloat32x4(static_cast<float>(MaximumValue - ZeroPoint));
  auto ZeroPointVector = MlasBroadcastInt32x4(ZeroPoint);

  // Holds 4 quantized 8bit values that will be packed into the output as packed
  // 4bit values.
  ...
  while (N >= 4) {
    ...
    MlasQuantizeLinearStore4PackedValues<UnpackedType>(IntegerVector,
                                                       &TmpOutput[0]);
  ...
  for (size_t n = 0; n < N; n++) {
    ...
    MlasQuantizeLinearStoreSingleValue<UnpackedType>(IntegerVector,
                                                     &TmpOutput[0]);
    MlasSetInt4Element<UnpackedType>(Output, n, TmpOutput[0]);
  }
}

void MLASCALL MlasQuantizeLinearS4Kernel(const float *Input, uint8_t *Output,
                                         size_t N, float Scale,
                                         int8_t ZeroPoint) {
  MlasQuantizeLinearInt4Kernel<true>(Input, Output, N, Scale, ZeroPoint);
}

void MLASCALL MlasQuantizeLinearU4Kernel(const float *Input, uint8_t *Output,
                                         size_t N, float Scale,
                                         int8_t ZeroPoint) {
  MlasQuantizeLinearInt4Kernel<false>(Input, Output, N, Scale, ZeroPoint);
}

void MLASCALL MlasQuantizeLinearS8Kernel(const float *Input, int8_t *Output,
                                         size_t N, float Scale,
                                         int8_t ZeroPoint) {
  MlasQuantizeLinearKernel<int8_t>(Input, Output, N, Scale, ZeroPoint);
}

void MLASCALL MlasQuantizeLinearU8Kernel(const float *Input, uint8_t *Output,
                                         size_t N, float Scale,
                                         uint8_t ZeroPoint) {
  MlasQuantizeLinearKernel<uint8_t>(Input, Output, N, Scale, ZeroPoint);
}

void MLASCALL MlasQuantizeLinearU16Kernel(const float *Input, uint16_t *Output,
                                          size_t N, float Scale,
                                          uint16_t ZeroPoint) {
  MlasQuantizeLinearKernel<uint16_t>(Input, Output, N, Scale, ZeroPoint);
}

void MLASCALL MlasQuantizeLinearS16Kernel(const float *Input, int16_t *Output,
                                          size_t N, float Scale,
                                          int16_t ZeroPoint) {
...
template <>
void MLASCALL MlasQuantizeLinear<int8_t>(const float *Input, int8_t *Output,
                                         size_t N, float Scale,
                                         int8_t ZeroPoint) {
#if defined(MLAS_TARGET_AMD64)
...
template <>
void MLASCALL MlasQuantizeLinear<uint8_t>(const float *Input, uint8_t *Output,
                                          size_t N, float Scale,
                                          uint8_t ZeroPoint) {
#if defined(MLAS_TARGET_AMD64)
...
template <>
void MLASCALL MlasQuantizeLinear<uint16_t>(const float *Input, uint16_t *Output,
                                           size_t N, float Scale,
                                           uint16_t ZeroPoint) {
#if defined(MLAS_TARGET_AMD64)
...
template <>
void MLASCALL MlasQuantizeLinear<int16_t>(const float *Input, int16_t *Output,
                                          size_t N, float Scale,
                                          int16_t ZeroPoint) {
#if defined(MLAS_TARGET_AMD64)
...
#if !defined(MLAS_TARGET_POWER)
template void MLASCALL MlasQuantizeLinear<int8_t>(const float *Input,
                                                  int8_t *Output, size_t N,
                                                  float Scale,
                                                  int8_t ZeroPoint);

template void MLASCALL MlasQuantizeLinear<uint8_t>(const float *Input,
                                                   uint8_t *Output, size_t N,
                                                   float Scale,
                                                   uint8_t ZeroPoint);

template void MLASCALL MlasQuantizeLinear<int16_t>(const float *Input,
                                                   int16_t *Output, size_t N,
                                                   float Scale,
                                                   int16_t ZeroPoint);

template void MLASCALL MlasQuantizeLinear<uint16_t>(const float *Input,
                                                    uint16_t *Output, size_t N,
                                                    float Scale,
                                                    uint16_t ZeroPoint);

template <bool Signed>
void MLASCALL MlasQuantizeLinearInt4(const float *Input, uint8_t *Output,
                                     size_t N, float Scale, int8_t ZeroPoint) {
  constexpr int32_t MinimumValue = Int4Traits<Signed>::Min;
  constexpr int32_t MaximumValue = Int4Traits<Signed>::Max;
  using UnpackedType = typename Int4Traits<Signed>::UnpackedType;
  ...
  for (size_t n = 0; n < N; n++) {
    float FloatValue =
      std::nearbyintf(Input[n] / Scale) + static_cast<float>(ZeroPoint);
...
void MLASCALL MlasRequantizeOutput(
  ...
  const __m128 PerMatrixScaleVector =
    PerColumnScale ? _mm_setzero_ps() : _mm_load1_ps(Scale);
  const __m128 MinimumValueVector =
    _mm_set1_ps(float(std::numeric_limits<OutputType>::lowest() - ZeroPoint));
  const __m128 MaximumValueVector =
    _mm_set1_ps(float(std::numeric_limits<OutputType>::max() - ZeroPoint));
  ...
  while (CountM-- > 0) {
    ...
    while (n >= 16) {
      ...
      if (bias != nullptr) {
        IntegerVector0 = _mm_add_epi32(
          IntegerVector0, _mm_loadu_si128((const __m128i *)&bias[0]));
        IntegerVector1 = _mm_add_epi32(
          IntegerVector1, _mm_loadu_si128((const __m128i *)&bias[4]));
        IntegerVector2 = _mm_add_epi32(
          IntegerVector2, _mm_loadu_si128((const __m128i *)&bias[8]));
        IntegerVector3 = _mm_add_epi32(
          IntegerVector3, _mm_loadu_si128((const __m128i *)&bias[12]));
        bias += 16;
      }
    ...
    while (n > 0) {
      ...
      if (n >= 4) {
        ...
        if (bias != nullptr) {
          IntegerVector = _mm_add_epi32(
            IntegerVector, _mm_loadu_si128((const __m128i *)&bias[0]));
          bias += 4;
        }
...
void MLASCALL MlasRequantizeOutput(
  ...
  bool PerColumnScale, OutputType ZeroPoint, size_t StartM, size_t StartN,
  size_t CountM, size_t CountN) {
  const float32x4_t PerMatrixScaleVector =
    PerColumnScale ? vdupq_n_f32(0) : vld1q_dup_f32(Scale);
  ...
  while (CountM-- > 0) {
    ...
    while (n >= 16) {
      ...
      if (bias != nullptr) {
        IntegerVector.val[0] =
          vaddq_s32(IntegerVector.val[0], vld1q_s32(&bias[0]));
        IntegerVector.val[1] =
          vaddq_s32(IntegerVector.val[1], vld1q_s32(&bias[4]));
        IntegerVector.val[2] =
          vaddq_s32(IntegerVector.val[2], vld1q_s32(&bias[8]));
        IntegerVector.val[3] =
          vaddq_s32(IntegerVector.val[3], vld1q_s32(&bias[12]));
        bias += 16;
      }
      ...
      if (scale != nullptr) {
        ...
        FloatVector.val[0] =
          vmulq_f32(FloatVector.val[0], PerColumnScaleVector.val[0]);
        FloatVector.val[1] =
          vmulq_f32(FloatVector.val[1], PerColumnScaleVector.val[1]);
        FloatVector.val[2] =
          vmulq_f32(FloatVector.val[2], PerColumnScaleVector.val[2]);
        FloatVector.val[3] =
          vmulq_f32(FloatVector.val[3], PerColumnScaleVector.val[3]);

      } else {

        FloatVector.val[0] =
          vmulq_f32(FloatVector.val[0], PerMatrixScaleVector);
        FloatVector.val[1] =
          vmulq_f32(FloatVector.val[1], PerMatrixScaleVector);
        FloatVector.val[2] =
          vmulq_f32(FloatVector.val[2], PerMatrixScaleVector);
        FloatVector.val[3] =
          vmulq_f32(FloatVector.val[3], PerMatrixScaleVector);
      }
      ...
      WordVector.val[0] =
        vqmovn_high_s32(vqmovn_s32(IntegerVector.val[0]), IntegerVector.val[1]);
      WordVector.val[1] =
        vqmovn_high_s32(vqmovn_s32(IntegerVector.val[2]), IntegerVector.val[3]);
      ...
      if (std::is_signed<OutputType>::value) {
        vst1q_s8(
          reinterpret_cast<int8_t *>(RowOutput),
          vqmovn_high_s16(vqmovn_s16(WordVector.val[0]), WordVector.val[1]));
      } else {
        vst1q_u8(
          reinterpret_cast<uint8_t *>(RowOutput),
    ...
    while (n > 0) {
      ...
      int16x8_t WordVector =
        vcombine_s16(vqmovn_s32(IntegerVector), vdup_n_s16(0));
      ...
      if (std::is_signed<OutputType>::value) {
        ByteVector =
          vcombine_u8(vreinterpret_u8_s8(vqmovn_s16(WordVector)), vdup_n_u8(0));
      } else {
...
void MLASCALL MlasRequantizeOutput(
  ...
  float MinimumValue =
    float(std::numeric_limits<OutputType>::lowest() - ZeroPoint);
  float MaximumValue =
    float(std::numeric_limits<OutputType>::max() - ZeroPoint);
  ...
  while (CountM-- > 0) {
    ...
    while (n > 0) {
      ...
      IntegerValue =
        int32_t(MlasBitsOfFp32(FloatValue + MLAS_ROUNDING_BIAS_MAGIC)) -
...
template <typename OutputType>
void MlasRequantizeOutput(const int32_t *Input, size_t InputLeadingDimension,
                          OutputType *Output, size_t OutputLeadingDimension,
                          const int32_t *Bias, const float *Scale,
                          bool PerColumnScale, OutputType ZeroPoint,
                          size_t StartM, size_t StartN, size_t CountM,
                          size_t CountN) {
  // TO BE CHECK
  ...
  const __m128 PerMatrixScaleVector =
    PerColumnScale ? MlasReinterpretAsFloat32x4(__lsx_vldi(0))
  ...
  while (CountM-- > 0) {
    ...
    while (n >= 16) {
      ...
      if (bias != nullptr) {
        ...
        IntegerVector3 = __lsx_vadd_w(IntegerVector3,
                                      __lsx_vld((const __m128i *)&bias[12], 0));
        bias += 16;
      }
      ...
      if (scale != nullptr) {
        ...
        FloatVector0 = __lsx_vfmul_s(
          FloatVector0,
          MlasReinterpretAsFloat32x4(__lsx_vld((__m128i *)&scale[0], 0)));
        FloatVector1 = __lsx_vfmul_s(
          FloatVector1,
          MlasReinterpretAsFloat32x4(__lsx_vld((__m128i *)&scale[4], 0)));
        FloatVector2 = __lsx_vfmul_s(
          FloatVector2,
          MlasReinterpretAsFloat32x4(__lsx_vld((__m128i *)&scale[8], 0)));
        FloatVector3 = __lsx_vfmul_s(
          FloatVector3,
          MlasReinterpretAsFloat32x4(__lsx_vld((__m128i *)&scale[12], 0)));
        scale += 16;
    ...
    while (n > 0) {
      ...
      if (n >= 4) {
        ...
        if (bias != nullptr) {
          IntegerVector = __lsx_vadd_w(IntegerVector,
                                       __lsx_vld((const __m128i *)&bias[0], 0));
          bias += 4;
        }
      ...
      if (scale != nullptr) {
        ...
        if (n >= 4) {
          ScaleVector =
            MlasReinterpretAsFloat32x4(__lsx_vld((__m128i *)scale, 0));
          scale += 4;
        } else {
...
template void MLASCALL MlasRequantizeOutput<int8_t>(
  const int32_t *Input, size_t InputLeadingDimension, int8_t *Output,
  size_t OutputLeadingDimension, const int32_t *Bias, const float *Scale,
  bool PerColumnScale, int8_t ZeroPoint, size_t StartM, size_t StartN,
  size_t CountM, size_t CountN);

template void MLASCALL MlasRequantizeOutput<uint8_t>(
  const int32_t *Input, size_t InputLeadingDimension, uint8_t *Output,
  size_t OutputLeadingDimension, const int32_t *Bias, const float *Scale,
  bool PerColumnScale, uint8_t ZeroPoint, size_t StartM, size_t StartN,
  size_t CountM, size_t CountN);
