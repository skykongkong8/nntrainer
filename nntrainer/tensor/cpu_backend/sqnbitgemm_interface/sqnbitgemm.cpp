...
enum SQNBitGemmVariant {
  ...
  // Keep this element last and ensure that its value is the number of valid
  // SQNBitGemmVariant values. Its value is used as an array size.
  SQNBitGemmVariantCount,
};

SQNBitGemmVariant
GetSQNBitGemmVariant(size_t BlkBitWidth, size_t BlkLen,
                     MLAS_SQNBIT_GEMM_COMPUTE_TYPE ComputeType) {
  if (BlkBitWidth == 4 && (BlkLen == 16 || BlkLen == 32 || BlkLen == 64 ||
                           BlkLen == 128 || BlkLen == 256)) {
...
bool MLASCALL
MlasIsSQNBitGemmAvailable(size_t BlkBitWidth, size_t BlkLen,
                          MLAS_SQNBIT_GEMM_COMPUTE_TYPE ComputeType) {
...
size_t
SQNBitGemmPerGemmWorkspaceSize(size_t M, size_t N, size_t K, size_t BlkBitWidth,
                               size_t BlkLen,
                               MLAS_SQNBIT_GEMM_COMPUTE_TYPE ComputeType) {
  const auto *Dispatch = GetMlasPlatform().SQNBitGemmDispatch;
  if (Dispatch == nullptr) {
  ...
  if (BlkBitWidth == 4 && Dispatch->SQ4BitGemmPerGemmWorkspaceSize != nullptr) {
    return Dispatch->SQ4BitGemmPerGemmWorkspaceSize(M, N, K, BlkLen,
                                                    ComputeType);
  }
...
size_t
SQNBitGemmPerGemmWorkspaceAlignment(size_t BlkBitWidth, size_t BlkLen,
                                    MLAS_SQNBIT_GEMM_COMPUTE_TYPE ComputeType) {
  const auto *Dispatch = GetMlasPlatform().SQNBitGemmDispatch;
  if (Dispatch == nullptr) {
    return 1;
  ...
  if (BlkBitWidth == 4 &&
      Dispatch->SQ4BitGemmPerGemmWorkspaceAlignment != nullptr) {
...
size_t
SQNBitGemmPerGemmWorkspaceStride(size_t M, size_t N, size_t K,
                                 size_t BlkBitWidth, size_t BlkLen,
                                 MLAS_SQNBIT_GEMM_COMPUTE_TYPE ComputeType) {
  const auto Size =
    SQNBitGemmPerGemmWorkspaceSize(M, N, K, BlkBitWidth, BlkLen, ComputeType);
  const auto Alignment =
    SQNBitGemmPerGemmWorkspaceAlignment(BlkBitWidth, BlkLen, ComputeType);
  return MlasDivRoundup(Size, Alignment) * Alignment;
}

} // namespace

size_t MLASCALL MlasSQNBitGemmBatchWorkspaceSize(
  size_t M, size_t N, size_t K, size_t BatchN, size_t BlkBitWidth,
  size_t BlkLen, MLAS_SQNBIT_GEMM_COMPUTE_TYPE ComputeType) {
  const size_t PerGemmWorkspaceStride =
    SQNBitGemmPerGemmWorkspaceStride(M, N, K, BlkBitWidth, BlkLen, ComputeType);
...
size_t MLASCALL MlasSQNBitGemmPackQuantBDataSize(
  size_t N, size_t K, size_t BlkBitWidth, size_t BlkLen,
  MLAS_SQNBIT_GEMM_COMPUTE_TYPE ComputeType) {
  const auto *Dispatch = GetMlasPlatform().SQNBitGemmDispatch;
  if (Dispatch == nullptr) {
    return 0;
  ...
  if (BlkBitWidth == 4 && Dispatch->SQ4BitGemmPackQuantBDataSize != nullptr) {
    return Dispatch->SQ4BitGemmPackQuantBDataSize(N, K, BlkLen, ComputeType);
  }
...
struct PerGemmQuantAWorkspace {
  PerGemmQuantAWorkspace(void *PerGemmWorkspace, size_t M, size_t BlockCountK,
                         size_t BlkLen) :
    PerGemmWorkspace_(PerGemmWorkspace),
    M_(M),
    BlockCountK_(BlockCountK),
    BlkLen_(BlkLen) {
...
void MLASCALL MlasSQNBitGemmPackQuantBData(
  size_t N, size_t K, size_t BlkBitWidth, size_t BlkLen,
  MLAS_SQNBIT_GEMM_COMPUTE_TYPE ComputeType, const void *QuantBData,
  void *PackedQuantBDataAndOrBlkSumWorkspace, const void *QuantBScale,
  bool has_zp_input, const void *QuantBZeroPoint, MLAS_THREADPOOL *ThreadPool) {
  const auto *Dispatch = GetMlasPlatform().SQNBitGemmDispatch;
  if (Dispatch == nullptr) {
    return;
  }
  ...
  if (BlkBitWidth == 4) {
    if (ComputeType == CompInt8 &&
        Dispatch->SQ4BitGemmPackQuantBDataAndBlkSum != nullptr) {
      const size_t BlockCountK = MlasDivRoundup(K, BlkLen);
      PackedQuantBDataStruct packed_quant_b(
        PackedQuantBDataAndOrBlkSumWorkspace, N, BlockCountK, BlkLen);
      Dispatch->SQ4BitGemmPackQuantBDataAndBlkSum(
        N, K, BlkLen, ComputeType, static_cast<const std::byte *>(QuantBData),
        static_cast<const float *>(QuantBScale), has_zp_input,
        static_cast<const std::byte *>(QuantBZeroPoint), packed_quant_b,
        ThreadPool);
    } else if (Dispatch->SQ4BitGemmPackQuantBData != nullptr) {
      // TODO: these assertions are true if called from matmul_nbits kernel but
      // not from mlas tests.
...
inline void AddBiasForGemm(const float *Bias, float *C, size_t CountM,
                           size_t CountN, size_t ldc) {
...
typedef void(SQNBitGemmFn)(size_t BlkLen, size_t K,
                           const MLAS_SQNBIT_GEMM_DATA_PARAMS *DataParams,
                           void *PerGemmWorkspace, size_t RangeStartM,
                           size_t RangeCountM, size_t RangeStartN,
                           size_t RangeCountN);

void SQ4BitGemm_CompFp32(const size_t BlkLen, const size_t K,
  ...
  const size_t k_blks_zp_bytes =
    MlasQNBitZeroPointsForBlksSizeInBytes<BlkBitWidth>(k_blks);
  ...
  const std::byte *QuantBZeroPoint =
    (DataParams->QuantBZeroPoint == nullptr)
      ...
      : static_cast<const std::byte *>(DataParams->QuantBZeroPoint) +
          RangeStartN * k_blks_zp_bytes;

  float *C = DataParams->C + RangeStartM * ldc + RangeStartN;

  const float *Bias =
    (DataParams->Bias == nullptr) ? nullptr : DataParams->Bias + RangeStartN;
  ...
  if (RangeCountM == 1) {
    ...
    for (size_t n = 0; n < RangeCountN; n += CountN) {
      ...
      const std::byte *b_col_zp = (QuantBZeroPoint == nullptr)
                                    ? nullptr
      ...
      GetMlasPlatform().SQNBitGemmDispatch->SQ4BitGemmM1Kernel_CompFp32(
        BlkLen, a_row, b_col, b_col_scale, b_col_zp, c_blk, CountN, K, k_blks,
        bias);

      if (DataParams->PostProcessor != nullptr) {
        DataParams->PostProcessor->Process(DataParams->C, RangeStartM,
                                           RangeStartN + n, RangeCountM, CountN,
                                           ldc);
      }
    }
    return;
  ...
  for (size_t n = 0; n < RangeCountN; n += CountN) {
    ...
    GetMlasPlatform().SQNBitGemmDispatch->Q4BitBlkDequantBForSgemm_CompFp32(
      BlkLen, dequant_b, b_col, b_col_scale, b_col_zp, CountN, K, k_blks);
  ...
  defined(MLAS_TARGET_LARCH64)
...
#else
      auto RowsHandled = MlasSgemmKernelZero(
        a_row, dequant_b, c_blk, K, RowsRemaining, CountN, lda, ldc, 1.f);
#endif
      ...
      if (DataParams->PostProcessor != nullptr) {
        DataParams->PostProcessor->Process(
          DataParams->C, RangeStartM + RangeCountM - RowsRemaining,
...
void SQ4BitGemm_CompInt8(const size_t BlkLen, const size_t K,
                         const MLAS_SQNBIT_GEMM_DATA_PARAMS *const DataParams,
                         void *const PerGemmWorkspace, const size_t RangeStartM,
                         const size_t RangeCountM, const size_t RangeStartN,
                         const size_t RangeCountN) {
  ...
  const size_t lda =
    k_blks *
    (per_gemm_quant_a_workspace->QuantScale ? BlkLen : Q8BlkSize(BlkLen));
  const size_t ldc = DataParams->ldc;
  const size_t ldb = k_blks * MlasQNBitBlkDataSizeInBytes(BlkBitWidth, BlkLen);
  const size_t k_blks_zp_bytes =
    MlasQNBitZeroPointsForBlksSizeInBytes<BlkBitWidth>(k_blks);
  ...
  const std::byte *QuantA =
    per_gemm_quant_a_workspace->QuantData + RangeStartM * lda;
  const float *QuantAScale =
    per_gemm_quant_a_workspace->QuantScale + RangeStartM * k_blks;
  ...
  const float *ABlockSum =
    per_gemm_quant_a_workspace->BlockSum + RangeStartM * k_blks;
  ...
  for (size_t n = 0; n < RangeCountN; n += CountN) {
    ...
    if (GetMlasPlatform().SQNBitGemmDispatch->SQ4BitGemmKernel_CompInt8 !=
      ...
      size_t RowsRemaining = RangeCountM;
      while (RowsRemaining > 0) {
        const auto RowsHandled =
          GetMlasPlatform().SQNBitGemmDispatch->SQ4BitGemmKernel_CompInt8(
            BlkLen, a_row, b_col, b_col_scale, b_col_zp, c_blk, RowsRemaining,
            CountN, K, k_blks, ldc, bias);
...
#ifdef MLAS_TARGET_AMD64_IX86
    else if (GetMlasPlatform()
               .SQNBitGemmDispatch->SQ4BitGemmKernel_BlkSum_CompInt8 !=
             nullptr) {
      const float *b_blk_sum = QuantBBlkSum + n * k_blks;
      GetMlasPlatform().SQNBitGemmDispatch->SQ4BitGemmKernel_BlkSum_CompInt8(
        BlkLen, QuantA, QuantAScale, b_col, b_col_scale, b_col_zp, c_blk,
        RangeCountM, CountN, K, k_blks, bias, ldc, ABlockSum, b_blk_sum);
...
typedef void(InitializeWorkspaceFn)(
  size_t M, size_t N, size_t K, size_t BatchN, size_t BlkLen,
  const MLAS_SQNBIT_GEMM_DATA_PARAMS *DataParams, void *Workspace,
  size_t PerGemmWorkspaceStride, MLAS_THREADPOOL *ThreadPool);

void InitializeWorkspace_CompInt8(
  size_t M, size_t N, size_t K, size_t BatchN, size_t BlkLen,
  const MLAS_SQNBIT_GEMM_DATA_PARAMS *DataParams, void *Workspace,
  size_t PerGemmWorkspaceStride, MLAS_THREADPOOL *ThreadPool) {
  ...
  const auto QuantizeARow =
    GetMlasPlatform().SQNBitGemmDispatch->QuantizeARow_CompInt8;
  const auto QuantizeARow2 =
    GetMlasPlatform().SQNBitGemmDispatch->QuantizeARowComputeBlkSum_CompInt8;
  ...
  if (QuantizeARow) {
    ...
    MlasTrySimpleParallel(ThreadPool, BatchN, [&](ptrdiff_t gemm_idx) {
      ...
      void *PerGemmWorkspace =
        static_cast<std::byte *>(Workspace) + gemm_idx * PerGemmWorkspaceStride;
      PerGemmQuantAWorkspace quant_a_data(PerGemmWorkspace, M, BlockCountK,
                                          BlkLen);
...
constexpr auto OperationMap = []() {
  ...
  ops[SQNBitGemmVariant_BitWidth4_CompInt8].InitializeWorkspace =
...
void MLASCALL MlasSQNBitGemmBatch(
  const size_t M, const size_t N, const size_t K, const size_t BatchN,
  const size_t BlkBitWidth, const size_t BlkLen,
  MLAS_SQNBIT_GEMM_COMPUTE_TYPE ComputeType,
  const MLAS_SQNBIT_GEMM_DATA_PARAMS *DataParams, void *Workspace,
  ...
  if (Workspace != nullptr) {
    ...
    Workspace = reinterpret_cast<void *>((WorkspaceAddress + Alignment - 1) &
                                         (~(Alignment - 1)));
  }
  ...
  if (const auto InitializeWorkspaceOperation =
        OperationMap[Variant].InitializeWorkspace;
      InitializeWorkspaceOperation != nullptr) {
    InitializeWorkspaceOperation(M, N, K, BatchN, BlkLen, DataParams, Workspace,
                                 PerGemmWorkspaceStride, ThreadPool);
  }
  ...
  if (ThreadPool == nullptr) {
    for (size_t gemm_i = 0; gemm_i < BatchN; gemm_i++) {
      ...
      if (ComputeType == CompInt8 &&
          GetMlasPlatform()
              .SQNBitGemmDispatch->SQ4BitGemmPackQuantBDataAndBlkSum !=
            nullptr) {
        PackedQuantBDataStruct packed_quant_b(
          const_cast<void *>(Data->QuantBDataWorkspace), N, BlockCountK,
          BlkLen);
        const_cast<MLAS_SQNBIT_GEMM_DATA_PARAMS *>(Data)->PackedQuantBData =
          packed_quant_b.PackedQuantBData;
        const_cast<MLAS_SQNBIT_GEMM_DATA_PARAMS *>(Data)->QuantBBlkSum =
          packed_quant_b.QuantBBlkSum;
        const_cast<MLAS_SQNBIT_GEMM_DATA_PARAMS *>(Data)->QuantBScale =
          packed_quant_b.PackedQuantBScale;
        PerGemmQuantAWorkspace per_gemm_quant_a_workspace(PerGemmWorkspace, M,
                                                          BlockCountK, BlkLen);
        ComputeOperation(BlkLen, K, Data, &per_gemm_quant_a_workspace, 0, M, 0,
                         N);
      } else {
  ...
  if (ThreadsPerGemm > 1) {
    ...
    if (max_nc < nc) {
      nc =
        std::min(nc, MlasDivRoundup(max_nc, MLAS_QGEMM_STRIDEN_THREAD_ALIGN) *
  ...
  MlasTrySimpleParallel(
    ThreadPool, ThreadsPerGemm * BatchN, [&](ptrdiff_t tid) {
      ...
      if (ComputeType == CompInt8 &&
        ...
        PerGemmQuantAWorkspace per_gemm_quant_a_workspace(PerGemmWorkspace, M,
                                                          BlockCountK, BlkLen);
        ComputeOperation(BlkLen, K, Data, &per_gemm_quant_a_workspace,
                         RangeStartM, RangeCountM, RangeStartN, RangeCountN);
      } else {
        ComputeOperation(BlkLen, K, Data, PerGemmWorkspace, RangeStartM,
