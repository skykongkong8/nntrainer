...
inline const float &Q8BlkScale(const std::byte *BlkPtr) {
  return *reinterpret_cast<const float *>(BlkPtr);
}

inline float &Q8BlkScale(std::byte *BlkPtr) {
  return *reinterpret_cast<float *>(BlkPtr);
}

inline const int8_t *Q8BlkData(const std::byte *BlkPtr) {
  return reinterpret_cast<const int8_t *>(BlkPtr + sizeof(float));
}

inline int8_t *Q8BlkData(std::byte *BlkPtr) {
  return reinterpret_cast<int8_t *>(BlkPtr + sizeof(float));
}

inline constexpr size_t Q8BlkSize(size_t BlkLen) {
...
inline constexpr size_t Q8BlkAlignment() { return alignof(float); }
