...
void MlasConvDepthwiseKernel(const InputType *const *Input,
                             InputType InputZeroPoint, const FilterType *Filter,
  ...
  for (size_t c = 0; c < Channels; ++c) {
    for (size_t out_idx = 0; out_idx < OutputCount; ++out_idx) {
      ...
      for (size_t k = 0; k < KernelSize; ++k) {
        ...
        int32_t in_val = static_cast<int32_t>(in_ptr[k]) -
                         static_cast<int32_t>(InputZeroPoint);
        int32_t fil_val = static_cast<int32_t>(filt_ptr[k]) -
                          static_cast<int32_t>(FilterZeroPoint);
        sum += in_val * fil_val;
      }
...
template void MlasConvDepthwiseKernel<uint8_t, uint8_t>(
  const uint8_t *const *Input, uint8_t InputZeroPoint, const uint8_t *Filter,
  uint8_t FilterZeroPoint, int32_t *Output, size_t Channels, size_t OutputCount,
  size_t KernelSize);
template void MlasConvDepthwiseKernel<uint8_t, int8_t>(
  const uint8_t *const *Input, uint8_t InputZeroPoint, const int8_t *Filter,
  int8_t FilterZeroPoint, int32_t *Output, size_t Channels, size_t OutputCount,
  size_t KernelSize);

// 3. Depthwise convolution AVX2 variant (calls baseline implementation)
template <typename InputType, typename FilterType>
void MlasConvDepthwiseKernelAvx2(const InputType *const *Input,
  ...
  MlasConvDepthwiseKernel<InputType, FilterType>(
    Input, InputZeroPoint, Filter, FilterZeroPoint, Output, Channels,
...
void MlasConvDepthwiseFloat_CHW(const MLAS_CONV_PARAMETERS *Parameters,
                                const float *Input, const float *Filter,
                                float *Output, const float *Zeros) {
  // Basic depthwise conv: assume 2D conv for simplicity
  size_t C =
    Parameters
      ->InputChannels; // number of channels (depthwise: equals output channels)
...
size_t MlasQ4GemmPackBSize(MLAS_BLK_QUANT_TYPE QType, size_t N, size_t K) {
  ...
  size_t bytes_per_column =
    (K + 1) / 2; // K values compressed into bytes (2 per byte)
...
void *MlasQ4GemmPackB(MLAS_BLK_QUANT_TYPE QType, void *PackedBuf,
                      const float *FpData, size_t N, size_t K, size_t ldb) {
  ...
  for (size_t j = 0; j < N; ++j) {
    for (size_t i = 0; i < K; i += 2) {
      ...
      int8_t qval1 = static_cast<int8_t>(
        lroundf(std::fmax(-8.f, std::fmin(7.f, FpData[i + j * ldb]))));
      int8_t qval2 = 0;
      if (i + 1 < K) {
        qval2 = static_cast<int8_t>(
          lroundf(std::fmax(-8.f, std::fmin(7.f, FpData[i + 1 + j * ldb]))));
      }
...
void MlasQ4GemmUnPackB(MLAS_BLK_QUANT_TYPE QType, const void *PackedBuf,
                       float *FpData, size_t N, size_t K, size_t ldb) {
...
void MlasQ8Q4GemmBatch(size_t M, size_t N, size_t K, const int8_t *A,
                       size_t lda, int8_t A_zero_point, const void *PackedB,
                       int8_t B_zero_point, int32_t *C, size_t ldc,
                       size_t batch_count) {
...
void MlasQ4GemmBatch( // (if needed, just call Q8Q4 implementation assuming A is
                      // also 8-bit)
  size_t M, size_t N, size_t K, const int8_t *A, size_t lda,
  int8_t A_zero_point, const void *PackedB, int8_t B_zero_point, int32_t *C,
  size_t ldc, size_t batch_count) {
  // Use the same implementation as Q8Q4
  MlasQ8Q4GemmBatch(M, N, K, A, lda, A_zero_point, PackedB, B_zero_point, C,
                    ldc, batch_count);
}
