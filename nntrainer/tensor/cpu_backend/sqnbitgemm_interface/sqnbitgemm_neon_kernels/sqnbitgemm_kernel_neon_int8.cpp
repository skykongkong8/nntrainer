...
inline void QuantizeBlock(size_t BlkLen, const float *A, size_t ElementCount,
                          std::byte *QuantA) {
  ...
  for (k = 0; k < ElementCount; k += SubBlkLen) {
    ...
    UnrolledLoop<SubBlkLen / 4>([&](size_t i) { abs_a[i] = vabsq_f32(a[i]); });
  ...
  for (k = 0; k < ElementCount; k += SubBlkLen) {
    ...
    UnrolledLoop<SubBlkLen / 4>(
      [&](size_t i) { a[i] = vmulq_n_f32(a[i], scale_reciprocal); });

    int32x4_t a_s32[SubBlkLen / 4];
    UnrolledLoop<SubBlkLen / 4>(
      [&](size_t i) { a_s32[i] = vcvtaq_s32_f32(a[i]); });

    UnrolledLoop<SubBlkLen / 4>([&](size_t i) {
      QuantAData[k + i * 4 + 0] =
        static_cast<int8_t>(vgetq_lane_s32(a_s32[i], 0));
      QuantAData[k + i * 4 + 1] =
        static_cast<int8_t>(vgetq_lane_s32(a_s32[i], 1));
      QuantAData[k + i * 4 + 2] =
        static_cast<int8_t>(vgetq_lane_s32(a_s32[i], 2));
      QuantAData[k + i * 4 + 3] =
        static_cast<int8_t>(vgetq_lane_s32(a_s32[i], 3));
    });
  }
  ...
  for (; k < BlkLen; k += SubBlkLen) {
    ...
    UnrolledLoop<SubBlkLen / 16>(
      [&](size_t i) { vst1q_s8(QuantAData + k + i * 16, Zeros); });
  }
}

} // namespace

void QuantizeARow_CompInt8(size_t BlkLen, const float *A, size_t CountK,
                           std::byte *QuantA) {
...
//
// The ComputeRxC functions compute an R row by C column tile of the output
// matrix.
//

template <bool HasZeroPoint>
inline void SQ4BitGemm_CompInt8_Compute4x2_BlkLen16(
  const std::byte *QuantARowPtr, const std::byte *QuantBDataColPtr,
  const float *QuantBScaleColPtr, const std::byte *QuantBZeroPointColPtr,
  const float *BiasPtr, float *SumPtr, size_t BlockCountK, size_t StrideQuantA,
  size_t StrideQuantBData, size_t StrideQuantBScale,
  size_t StrideQuantBZeroPoint, size_t ldc) {
  ...
  float32x4_t acc00{}, acc01{}, acc10{}, acc11{}, acc20{}, acc21{}, acc30{},
    acc31{};
  ...
  for (size_t k_blk_idx = 0; k_blk_idx < BlockCountK; ++k_blk_idx) {
    ...
    if constexpr (HasZeroPoint) {
      ...
      const std::byte QuantBZeroPointByteCol1 =
        *(QuantBZeroPointPtr + StrideQuantBZeroPoint);
      if ((k_blk_idx & 1) == 0) {
        bzp_col0 =
          std::to_integer<int8_t>(QuantBZeroPointByteCol0 & std::byte{0x0F});
        bzp_col1 =
          std::to_integer<int8_t>(QuantBZeroPointByteCol1 & std::byte{0x0F});
      } else {
    ...
    // TODO handling only 16 elements per accumulator at a time here, probably
    // can do better
    {
      // load B
      const uint8x8_t bv_packed_col0 =
        vld1_u8(reinterpret_cast<const uint8_t *>(QuantBDataPtr));
      const uint8x8_t bv_packed_col1 = vld1_u8(
        reinterpret_cast<const uint8_t *>(QuantBDataPtr) + StrideQuantBData);

      const uint8x8_t LowMaskU8x8 = vdup_n_u8(0x0F);

      int8x16_t bv_col0 = vreinterpretq_s8_u8(vcombine_u8(
        vand_u8(bv_packed_col0, LowMaskU8x8), vshr_n_u8(bv_packed_col0, 4)));
      int8x16_t bv_col1 = vreinterpretq_s8_u8(vcombine_u8(
        vand_u8(bv_packed_col1, LowMaskU8x8), vshr_n_u8(bv_packed_col1, 4)));
...
template <bool HasZeroPoint>
inline void SQ4BitGemm_CompInt8_Compute4x2_BlkLenGreaterThan16(
  size_t BlkLen, const std::byte *QuantARowPtr,
  const std::byte *QuantBDataColPtr, const float *QuantBScaleColPtr,
  const std::byte *QuantBZeroPointColPtr, const float *BiasPtr, float *SumPtr,
  size_t BlockCountK, size_t StrideQuantA, size_t StrideQuantBData,
  size_t StrideQuantBScale, size_t StrideQuantBZeroPoint, size_t ldc) {
  ...
  for (size_t k_blk_idx = 0; k_blk_idx < BlockCountK; ++k_blk_idx) {
    ...
    for (size_t sub_blk_idx = 0; sub_blk_idx < SubBlksPerBlk; ++sub_blk_idx) {
      // load B
      const uint8x16_t bv_packed_col0 =
        vld1q_u8(reinterpret_cast<const uint8_t *>(QuantBDataPtr));
      const uint8x16_t bv_packed_col1 = vld1q_u8(
        reinterpret_cast<const uint8_t *>(QuantBDataPtr) + StrideQuantBData);

      const uint8x16_t LowMaskU8x16 = vdupq_n_u8(0x0F);

      int8x16_t bv_col0_0 =
        vreinterpretq_s8_u8(vandq_u8(bv_packed_col0, LowMaskU8x16));
      int8x16_t bv_col0_1 = vreinterpretq_s8_u8(vshrq_n_u8(bv_packed_col0, 4));
      int8x16_t bv_col1_0 =
        vreinterpretq_s8_u8(vandq_u8(bv_packed_col1, LowMaskU8x16));
      ...
      // rows 0 and 1 of A
      {
        ...
        // quantized dot product
        const int32x4_t dot00 = vdotq_s32(
          vdotq_s32(int32x4_t{}, av_row0_0, bv_col0_0), av_row0_1, bv_col0_1);
        const int32x4_t dot01 = vdotq_s32(
          vdotq_s32(int32x4_t{}, av_row0_0, bv_col1_0), av_row0_1, bv_col1_1);
        const int32x4_t dot10 = vdotq_s32(
          vdotq_s32(int32x4_t{}, av_row1_0, bv_col0_0), av_row1_1, bv_col0_1);
        const int32x4_t dot11 = vdotq_s32(
          vdotq_s32(int32x4_t{}, av_row1_0, bv_col1_0), av_row1_1, bv_col1_1);
      ...
      // rows 2 and 3 of A
      {
        ...
        // quantized dot product
        const int32x4_t dot20 = vdotq_s32(
          vdotq_s32(int32x4_t{}, av_row2_0, bv_col0_0), av_row2_1, bv_col0_1);
        const int32x4_t dot21 = vdotq_s32(
          vdotq_s32(int32x4_t{}, av_row2_0, bv_col1_0), av_row2_1, bv_col1_1);
        const int32x4_t dot30 = vdotq_s32(
          vdotq_s32(int32x4_t{}, av_row3_0, bv_col0_0), av_row3_1, bv_col0_1);
        const int32x4_t dot31 = vdotq_s32(
          vdotq_s32(int32x4_t{}, av_row3_0, bv_col1_0), av_row3_1, bv_col1_1);
...
template <bool HasZeroPoint>
inline void SQ4BitGemm_CompInt8_Compute1x1_BlkLen16(
  const std::byte *QuantARowPtr, const std::byte *QuantBDataColPtr,
  const float *QuantBScaleColPtr, const std::byte *QuantBZeroPointColPtr,
  const float *BiasPtr, float *SumPtr, size_t BlockCountK) {
  constexpr size_t BlkLen = 16;
  ...
  for (; k_blks_remaining > 1; k_blks_remaining -= 2) {
    ...
    const float32x4_t scale0 =
      vdupq_n_f32(Q8BlkScale(QuantABlk0) * QuantBScalePtr[0]);
    const float32x4_t scale1 =
      vdupq_n_f32(Q8BlkScale(QuantABlk1) * QuantBScalePtr[1]);
    ...
    int8x16_t bv0 = vreinterpretq_s8_u8(
      vcombine_u8(vget_low_u8(bv_lo01), vget_low_u8(bv_hi01)));
    int8x16_t bv1 = vreinterpretq_s8_u8(
      vcombine_u8(vget_high_u8(bv_lo01), vget_high_u8(bv_hi01)));
  ...
  if (k_blks_remaining > 0) {
    ...
    // compute combined scale
    const float32x4_t scale0 =
      vdupq_n_f32(Q8BlkScale(QuantABlk0) * (*QuantBScalePtr));
...
template <bool HasZeroPoint>
inline void SQ4BitGemm_CompInt8_Compute1x1_BlkLen32(
  const std::byte *QuantARowPtr, const std::byte *QuantBDataColPtr,
  const float *QuantBScaleColPtr, const std::byte *QuantBZeroPointColPtr,
  const float *BiasPtr, float *SumPtr, size_t BlockCountK) {
  ...
  for (; k_blks_remaining > 1; k_blks_remaining -= 2) {
    ...
    // load B
    const uint8x16_t bv_packed0 =
      vld1q_u8(reinterpret_cast<const uint8_t *>(QuantBDataPtr));
    const uint8x16_t bv_packed1 =
      vld1q_u8(reinterpret_cast<const uint8_t *>(QuantBDataPtr) + 16);
    ...
    // quantized dot product
    const int32x4_t dot0 =
      vdotq_s32(vdotq_s32(int32x4_t{}, av_lo0, bv_lo0), av_hi0, bv_hi0);
    const int32x4_t dot1 =
      vdotq_s32(vdotq_s32(int32x4_t{}, av_lo1, bv_lo1), av_hi1, bv_hi1);
...
template <bool HasZeroPoint>
inline void SQ4BitGemm_CompInt8_Compute1x1_BlkLenGreaterThan32(
  size_t BlkLen, const std::byte *QuantARowPtr,
  const std::byte *QuantBDataColPtr, const float *QuantBScaleColPtr,
  const std::byte *QuantBZeroPointColPtr, const float *BiasPtr, float *SumPtr,
  size_t BlockCountK) {
  ...
  for (size_t k_blk_idx = 0; k_blk_idx < BlockCountK; ++k_blk_idx) {
    ...
    for (size_t sub_blk_idx = 0; sub_blk_idx < SubBlksPerBlk;
         sub_blk_idx += 2) {
      // load A
      ...
      // quantized dot product
      const int32x4_t dot0 =
        vdotq_s32(vdotq_s32(int32x4_t{}, av0, bv0), av1, bv1);
      const int32x4_t dot1 =
        vdotq_s32(vdotq_s32(int32x4_t{}, av2, bv2), av3, bv3);
...
inline void AdvanceColPtrs(size_t StrideQuantBData, size_t StrideQuantBScale,
                           ...
                           const float *&BiasPtr, float *&SumPtr) {
...
inline void AdvanceRowPtrs(size_t StrideQuantA, size_t ldc,
                           const std::byte *&QuantARowPtr, float *&SumRowPtr) {
  QuantARowPtr += NumRows * StrideQuantA;
  SumRowPtr += NumRows * ldc;
}

template <bool HasZeroPoint>
void SQ4BitGemmKernel_CompInt8_BlkLen16(const std::byte *QuantA,
                                        const std::byte *QuantBData,
                                        const float *QuantBScale,
                                        const std::byte *QuantBZeroPoint,
                                        float *C, size_t CountM, size_t CountN,
                                        size_t BlockCountK, size_t ldc,
                                        const float *Bias) {
  ...
  while (m_remaining > 3) {
    ...
    while (n_remaining > 1) {
      ...
      SQ4BitGemm_CompInt8_Compute4x2_BlkLen16<HasZeroPoint>(
        QuantARowPtr, QuantBDataColPtr, QuantBScaleColPtr,
        QuantBZeroPointColPtr, BiasPtr, SumPtr, BlockCountK, StrideQuantA,
        StrideQuantBData, StrideQuantBScale, StrideQuantBZeroPoint, ldc);

      // Move to next 2 columns
      AdvanceColPtrs<2, HasZeroPoint>(StrideQuantBData, StrideQuantBScale,
                                      StrideQuantBZeroPoint, QuantBDataColPtr,
                                      QuantBScaleColPtr, QuantBZeroPointColPtr,
                                      BiasPtr, SumPtr);
    ...
    if (n_remaining > 0) {
      ...
      for (size_t i = 0; i < 4; ++i) {
        SQ4BitGemm_CompInt8_Compute1x1_BlkLen16<HasZeroPoint>(
          QuantARowPtr + StrideQuantA * i, QuantBDataColPtr, QuantBScaleColPtr,
          QuantBZeroPointColPtr, BiasPtr, SumPtr + ldc * i, BlockCountK);
      }
    }

    // Move to next 4 rows
    AdvanceRowPtrs<4>(StrideQuantA, ldc, QuantARowPtr, SumRowPtr);
  ...
  while (m_remaining > 0) {
    ...
    while (n_remaining > 0) {
      ...
      SQ4BitGemm_CompInt8_Compute1x1_BlkLen16<HasZeroPoint>(
        QuantARowPtr, QuantBDataColPtr, QuantBScaleColPtr,
        QuantBZeroPointColPtr, BiasPtr, SumPtr, BlockCountK);

      // Move to next column
      AdvanceColPtrs<1, HasZeroPoint>(StrideQuantBData, StrideQuantBScale,
                                      StrideQuantBZeroPoint, QuantBDataColPtr,
                                      QuantBScaleColPtr, QuantBZeroPointColPtr,
                                      BiasPtr, SumPtr);
    ...
    // Move to next row
    AdvanceRowPtrs<1>(StrideQuantA, ldc, QuantARowPtr, SumRowPtr);
...
template <bool HasZeroPoint>
void SQ4BitGemmKernel_CompInt8_BlkLen32(const std::byte *QuantA,
                                        const std::byte *QuantBData,
                                        const float *QuantBScale,
                                        const std::byte *QuantBZeroPoint,
                                        float *C, size_t CountM, size_t CountN,
  ...
  while (m_remaining > 3) {
    ...
    while (n_remaining > 1) {
      ...
      SQ4BitGemm_CompInt8_Compute4x2_BlkLenGreaterThan16<HasZeroPoint>(
        BlkLen, QuantARowPtr, QuantBDataColPtr, QuantBScaleColPtr,
        QuantBZeroPointColPtr, BiasPtr, SumPtr, BlockCountK, StrideQuantA,
        StrideQuantBData, StrideQuantBScale, StrideQuantBZeroPoint, ldc);
...
template <bool HasZeroPoint>
void SQ4BitGemmKernel_CompInt8_BlkLenGreaterThan32(
  size_t BlkLen, const std::byte *QuantA, const std::byte *QuantBData,
  const float *QuantBScale, const std::byte *QuantBZeroPoint, float *C,
  size_t CountM, size_t CountN, size_t BlockCountK, size_t ldc,
  const float *Bias) {
  constexpr size_t BlkBitWidth = 4;
    ...
    if (n_remaining > 0) {
      ...
      for (size_t i = 0; i < 4; ++i) {
        SQ4BitGemm_CompInt8_Compute1x1_BlkLenGreaterThan32<HasZeroPoint>(
          BlkLen, QuantARowPtr + StrideQuantA * i, QuantBDataColPtr,
          QuantBScaleColPtr, QuantBZeroPointColPtr, BiasPtr, SumPtr + ldc * i,
          BlockCountK);
      }
    }
...
template <bool HasZeroPoint>
void SQ4BitGemmKernel_CompInt8_DispatchOnBlkLen(
  size_t BlkLen, const std::byte *QuantA, const std::byte *QuantBData,
  const float *QuantBScale, const std::byte *QuantBZeroPoint, float *C,
  size_t CountM, size_t CountN, size_t BlockCountK, size_t ldc,
  const float *Bias) {
  if (BlkLen == 16) {
    SQ4BitGemmKernel_CompInt8_BlkLen16<HasZeroPoint>(
      QuantA, QuantBData, QuantBScale, QuantBZeroPoint, C, CountM, CountN,
      BlockCountK, ldc, Bias);
  } else if (BlkLen == 32) {
    ...
    SQ4BitGemmKernel_CompInt8_BlkLenGreaterThan32<HasZeroPoint>(
      BlkLen, QuantA, QuantBData, QuantBScale, QuantBZeroPoint, C, CountM,
      CountN, BlockCountK, ldc, Bias);
  }
}

} // namespace

size_t SQ4BitGemmKernel_CompInt8(size_t BlkLen, const std::byte *QuantA,
                                 const std::byte *QuantBData,
                                 const float *QuantBScale,
                                 const std::byte *QuantBZeroPoint, float *C,
