...
struct BlockwiseQuantizer {
    ...
    static inline void quantizedBufferSizes(
...
struct BlockwiseQDQQuantizer<Tin, 4, signed_quant> {
    ...
    template <bool add8>
    static inline uint8_t Pack(uint8_t v0, uint8_t v1)
    {
        if constexpr (add8) {
    ...
    template <bool add8>
    static inline void Transpose(uint8_t src0, uint8_t src1, uint8_t& dst0, uint8_t& dst1)
    {
        if constexpr (add8) {
    ...
    static inline uint8_t QuantizeV(Tin src, float reciprocal_scale, uint8_t zero_point)
    {
