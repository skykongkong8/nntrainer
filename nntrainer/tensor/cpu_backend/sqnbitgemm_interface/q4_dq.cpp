...
template <typename T> constexpr size_t BlkQ4BufSize(size_t N, size_t K) {
  const size_t KBlocks = MlasDivRoundup(K, T::BlkLen);
  return N * KBlocks * T::BlobSize;
}

size_t MLASCALL MlasQ4GemmPackBSize(MLAS_BLK_QUANT_TYPE QType, size_t N,
                                    size_t K) {
...
template <typename T>
inline void MlasQ4GemmPackBImpl(void *PackedBuf, const float *FpData, size_t N,
                                size_t K, size_t ldb) {
    ...
    for (size_t k = 0; k < K; k += T::BlkLen) {
      ...
      for (size_t kk = 0; kk < klen; kk += 32) {
        ...
        for (size_t l = 0; l < 16; l++) {
          const float v0 =
            l < kklen ? src[ldb * (kk + l)] * reciprocal_scale : 0;
          const uint8_t vi0 =
            (uint8_t)std::min(15.0f, std::max(0.0f, v0 + 8.5f));

          const size_t l1 = l + 16;
          const float v1 =
            (l1 < kklen) ? src[ldb * (kk + l1)] * reciprocal_scale : 0;
          const uint8_t vi1 =
            (uint8_t)std::min(15.0f, std::max(0.0f, v1 + 8.5f));
...
template <>
inline void MlasQ4GemmPackBImpl<MLAS_Q4TYPE_BLK1>(void *PackedBuf,
                                                  const float *FpData, size_t N,
                                                  size_t K, size_t ldb) {
  auto *dst_ptr = reinterpret_cast<uint8_t *>(PackedBuf);
...
void MLASCALL MlasQ4GemmPackB(MLAS_BLK_QUANT_TYPE QType, void *PackedBuf,
                              const float *FpData, size_t N, size_t K,
                              size_t ldb) {
  switch (QType) {
  case BlkQ4Sym:
...
template <typename T>
inline void MlasQ4GemmUnPackBImpl(float *FpData, const void *PackedBuf,
...
template <>
inline void
MlasQ4GemmUnPackBImpl<MLAS_Q4TYPE_BLK1>(float *FpData, const void *PackedBuf,
                                        size_t N, size_t K, size_t ldb) {
  const auto *src = reinterpret_cast<const uint8_t *>(PackedBuf);
  for (size_t n = 0; n < N; n++) {
    for (size_t k = 0; k < K; k += MLAS_Q4TYPE_BLK1::BlkLen) {
...
void MLASCALL MlasQ4GemmUnPackB(MLAS_BLK_QUANT_TYPE QType, float *FpData,
                                const void *PackedBuf, size_t N, size_t K,
                                size_t ldb) {
  switch (QType) {
  case BlkQ4Sym:
    return MlasQ4GemmUnPackBImpl<MLAS_Q4TYPE_BLK0>(FpData, PackedBuf, N, K,
                                                   ldb);
  case BlkQ4Sym64:
    return MlasQ4GemmUnPackBImpl<MLAS_Q4TYPE_BLK2>(FpData, PackedBuf, N, K,
                                                   ldb);
  case BlkQ4Sym128:
    return MlasQ4GemmUnPackBImpl<MLAS_Q4TYPE_BLK4>(FpData, PackedBuf, N, K,
                                                   ldb);
  default:
    return MlasQ4GemmUnPackBImpl<MLAS_Q4TYPE_BLK1>(FpData, PackedBuf, N, K,
                                                   ldb);
  }
}
...
template <int qbits, bool signed_quant> struct BitsTraits {
  ...
  static constexpr int kMax =
    signed_quant ? (1 << (qbits - 1)) - 1 : (1 << qbits) - 1;
  ...
  static constexpr int kPackSize = (qbits == 8)   ? 1
                                   : (qbits == 4) ? 2
                                   : (qbits == 2) ? 4
                                                  : 0;
  static_assert(kPackSize != 0,
                "Packing to whole bytes not supported for this qbits!");
};

/**
 * @brief Rectify min/max from a set of weights, and convert to scale and zero
 * point for quantization.
...
inline void range2scalezp(float min, float max, ScaleT &scale, uint8_t &zp) {
  min = std::min(min, 0.0f);
  max = std::max(max, 0.0f);
 ...
 */
template <typename ScaleT, int qbits, bool signed_quant>
inline void range2scale(float min, float max, ScaleT &scale) {
...
struct BlockwiseQuantizer {
  ...
  using QuantBlk = std::conditional_t<Columnwise, Shape2D<block_size, 1>,
                                      Shape2D<1, block_size>>;
  using ThreadBlk =
    Shape2D<QuantBlk::kRow * BitsTraits<qbits, false>::kPackSize,
            QuantBlk::kColumn>;

  static inline void quantizeMetaShape(int rows, int columns, int &meta_rows,
                                       int &meta_cols) {
    meta_rows = (rows + QuantBlk::kRow - 1) / QuantBlk::kRow;
    meta_cols = (columns + QuantBlk::kColumn - 1) / QuantBlk::kColumn;
  }

  static inline void quantizedShape(int rows, int columns, int &q_rows,
                                    int &q_cols) {
    int meta_rows;
    int meta_cols;
  ...
  static inline void quantizedBufferSizes(int rows, int columns,
                                          size_t &data_bytes,
                                          size_t &scale_num_elements,
    ...
    if (zero_point_bytes) {
      // this works for qbits == 4 but may need to be updated for other qbits
      // values
   ...
   * @param[out] dst           pointer to the quantized weights, column major:
   * [columns, rows]
   * @param[out] scale         pointer to the scales, column major:
   * [columns/QuantBlk::kColumn, rows/QuantBlk::kRow]
   * @param[out] zero_points   pointer to the zero points, same shape as scale
   * @param[in]  src           pointer to the source matrix, row major: [rows,
   * columns]
   * @param rows
   * @param columns
   * @param leadingDimension   stride of the source matrix, i.e. distance from
   * one row to the next
   */
  static void quantizeAndTranspose(uint8_t *dst, ElementT *scales,
                                   uint8_t *zero_points, const ElementT *src,
                                   int32_t rows, int32_t columns,
                                   int32_t leadingDimension,
                                   MLAS_THREADPOOL *thread_pool) {
    // Thread partitioning
    const auto thrd_row_blks = (rows + ThreadBlk::kRow - 1) / ThreadBlk::kRow;
    const auto thrd_col_blks =
      (columns + ThreadBlk::kColumn - 1) / ThreadBlk::kColumn;
    ...
    MlasTryBatchParallel(
      thread_pool, total_thrd_blks, [&](ptrdiff_t block_idx) {
        uint8_t zp_bytes[BitsTraits<qbits, false>::kPackSize];
        std::fill_n(zp_bytes, BitsTraits<qbits, false>::kPackSize, (uint8_t)8);

        const int32_t r_blk_idx =
          static_cast<int32_t>(block_idx / thrd_col_blks);
        const int32_t c_blk_idx =
          static_cast<int32_t>(block_idx % thrd_col_blks);
        ...
        for (int kpack = 0; kpack < BitsTraits<qbits, false>::kPackSize;
             kpack++) {
          ...
          if (row_start < row_end) {
            ...
            if (zero_points == nullptr) {
              ...
              range2scalezp<ElementT, qbits, false>(min, max, scales[meta_idx],
                                                    zp_bytes[kpack]);
            }
          }
        }
        ...
        if (zero_points != nullptr) {
          const int32_t meta_idx =
            meta_col * ((row_blks + 1) / 2) + meta_row / 2;
        ...
        for (int32_t j = c; j < c_end; ++j) {
          ...
          for (int32_t i = r; i < r_end; i += 2) {
            ...
            const float scale =
              static_cast<float>(scales[meta_c * row_blks + meta_r]);
            const float reciprocal_scale = scale ? 1.0f / scale : 0.0f;
            ...
            const uint8_t vi0 =
              (uint8_t)std::clamp(roundf(v0 * reciprocal_scale + zp), 0.0f,
            ...
            if (i + 1 < r_end) {
              ...
              const float v1 =
                static_cast<float>(src[(i + 1) * leadingDimension + j]);
              vi1 = (uint8_t)std::clamp(roundf(v1 * reciprocal_scale1 + zp1),
  ...
  /**
   * @brief Dequantize a column major quantized matrix, and store the result in
   * a column major matrix for use in GEMM
   * @param[out] dst           pointer to the dequantized matrix, column major:
   * [columns, rows]
   * @param[in]  weights       pointer to the quantized weights, column major:
   * [columns, rows]
   * @param[in]  scales        pointer to the scales of quantized blocks, column
   * major layout
   * @param[in]  zero_points   pointer to the zero points of quantized blocks,
   * packed column major scales
   * @param[in]  rows
   * @param[in]  columns
   */
  static void dequantize(ElementT *dst, const uint8_t *weights,
                         const ElementT *scales, const uint8_t *zero_points,
                         int32_t rows, int32_t columns,
                         MLAS_THREADPOOL *thread_pool) {
    // Thread partitioning
    const auto thrd_row_blks = (rows + ThreadBlk::kRow - 1) / ThreadBlk::kRow;
            ...
            if ((i + 1) < r_end) {
              ...
              if constexpr (QuantBlk::kRow == 1) {
                scale1 = static_cast<float>(
                  scales[meta_col * row_blks + meta_row + 1]);
                zp1 = (zp_pair >> 4) & 0xf;
              }
...
/**
 * @brief Blockwise quantization methods for QDQ format. Input tensor is
 * quantized along column or row. Scales and zeros are calculated. Based on
 * qbits, consecutive quantized elements in memory are packed together, which
 * means the packing is along the row. Quantized data are stored in row major,
 * so the output tensor reserves same shape, in terms of qbits type, as the
 * input tensor. If has zero points, quantized type is unsigned. Otherwise,
 * quantized type is signed and the zero point is 0. The transposed outputs are
 * used by MatMulNBits, so quant type becomes uint4 with default zp at 8.
...
struct BlockwiseQDQQuantizer<Tin, 4, signed_quant> {
  static inline uint8_t GetElem(uint8_t val, int32_t idx) {
    return (val >> (idx << 2)) & 0xF;
  }

  static inline uint8_t SetElem(uint8_t val, int32_t idx, uint8_t dst) {
    auto shift = idx << 2;
    return ((val & 0xF) << shift) | (dst & (~(0xF << shift)));
  }

  template <bool add8> static inline uint8_t Pack(uint8_t v0, uint8_t v1) {
    if constexpr (add8) {
  ...
  template <bool add8>
  static inline void Transpose(uint8_t src0, uint8_t src1, uint8_t &dst0,
                               uint8_t &dst1) {
    if constexpr (add8) {
  ...
  static inline uint8_t QuantizeV(Tin src, float reciprocal_scale,
                                  uint8_t zero_point) {
    return static_cast<uint8_t>(std::clamp(
      static_cast<int32_t>(
        std::roundf(static_cast<float>(src) * reciprocal_scale)) +
        static_cast<int32_t>(zero_point),
      BitsTraits<4, signed_quant>::kMin, BitsTraits<4, signed_quant>::kMax));
  }

  /**
   * @brief Quantize a matrix shape [rows, columns] column-wise. Scales and zero
   * points are calculated. Quantized data are packed row-wise based on qbits.
   * Quantized data are stored in row major so the output tensor reserves the
   * shape, in terms output type.
   * @param src               the source matrix, row major: [rows * columns]
   * @param scales            the scales of quantized blocks, row major with
   * shape: [ceil(rows/quant_block_size) * columns]
   * @param zero_points       the zero points of quantized blocks, packed. Same
   * shape as scales in terms of output type. In uint8_t, the shape is:
   *                          [ceil(columns * ceil(rows / quant_block_size) *
   * qbits / 8)]
   * @param dst               the quantized weights, row major: [rows * columns]
   * in terms of output type. In uint8_t, the shape is: [ceil(rows * columns *
   * qbits / 8]
   ...
   */
  static void QuantizeColumnWise(const Tin *src, Tin *scales,
                                 uint8_t *zero_points, uint8_t *dst,
                                 int32_t rows, int32_t columns,
                                 int32_t quant_block_size,
                                 MLAS_THREADPOOL *thread_pool) {
    ORT_ENFORCE(zero_points || signed_quant,
                "Unsigned quant with no zero points is not supported.");
    // Must avoid multiple thread write to a single byte, which means the
    // starting index of a thread block must be even. To achieve that, we need
    // to customize the thread block size based on the parity of columns.
    if (columns & 1) {
      QuantizeColumnWisePackUnaligned(src, scales, zero_points, dst, rows,
                                      columns, quant_block_size, thread_pool);
    } else {
      QuantizeColumnWisePackAligned(src, scales, zero_points, dst, rows,
                                    columns, quant_block_size, thread_pool);
    }
  }

  /**
   * @brief Transpose quantized tensors, which has been column-wise quantized,
   * for use in MatMulNbits. Since both src tensor and dst tensor are packed,
   * it's not needed to consider sign during the unpacking/packing in transpose.
   * @param src_weights       The quantized weights, row major: [rows, columns]
   * in qbits type. In uint8_t, size of [ceil(rows * columns * qbits / 8)].
   * @param src_scales        [ceil(rows / quant_block_size), columns]
   * @param src_zero_points   [ceil(rows / quant_block_size), columns] in qbits
   * type. In uint8_t, size of [ceil(ceil(rows / quant_block_size) * columns *
   * qbits / 8 )].
   * @param dst_weights       the transposed quantized weights, column major. In
   * uint8_t, the shape is [columns, ceil(rows / quant_block_size),
   * ceil(quant_block_size * qbits / 8)]
  ...
  static void TransposeColumnWiseQuantized(
    const uint8_t *src_weights, const Tin *src_scales,
    const uint8_t *src_zero_points, uint8_t *dst_weights, Tin *dst_scales,
    uint8_t *dst_zero_points, int32_t rows, int32_t columns,
    int32_t quant_block_size, MLAS_THREADPOOL *thread_pool) {
    ORT_ENFORCE(src_zero_points || signed_quant || dst_zero_points,
                "Unsigned quant types without zero points must allocate zero "
                "points with value 0.");
    // Must avoid multiple thread write to a single byte, which means the
    // starting index of a thread block must be even. To achieve that, we need
    // to customize the thread block size based on the parity of columns.
    if (columns & 1) {
      TransposeColumnWiseQuantizedPackUnaligned(
        src_weights, src_scales, src_zero_points, dst_weights, dst_scales,
        dst_zero_points, rows, columns, quant_block_size, thread_pool);
    } else {
...
private:
  static void QuantizeColumnWisePackAligned(const Tin *src, Tin *scales,
                                            uint8_t *zero_points, uint8_t *dst,
                                            int32_t rows, int32_t columns,
                                            int32_t quant_block_size,
                                            MLAS_THREADPOOL *thread_pool) {
    ORT_ENFORCE(columns % 2 == 0, "Columns must be multiple of 2.");
    // Thread block is [quant_block_size, thread_blk_size]. thread_blk_size % 2
    // == 0.
    constexpr int32_t thread_blk_size = 128;
    const auto num_row_thread_blk =
      (rows + quant_block_size - 1) / quant_block_size;
    const auto num_col_thread_blk =
      (columns + thread_blk_size - 1) / thread_blk_size;
    ...
    MlasTryBatchParallel(
      ...
      [&](ptrdiff_t thread_blk_idx) {
        ...
        ORT_ENFORCE(buffer_size == thread_blk_size,
                    "buffer size must be equal to thread block size.");
        float reciprocal_scale_t[buffer_size];
        uint8_t zp_t[buffer_size];
        float vmin_t[buffer_size];
        float vmax_t[buffer_size];

        const int32_t row_thread_blk_idx =
          static_cast<int32_t>(thread_blk_idx / num_col_thread_blk);
        const int32_t col_thread_blk_idx =
          static_cast<int32_t>(thread_blk_idx % num_col_thread_blk);
        ...
        // calculate min/max
        for (int32_t j = 0, input_idx_t = input_idx; j < row_size;
             ++j, input_idx_t += columns) {
          // TODO(fajin): use SIMD
        ...
        for (int32_t i = 0; i < col_size; i += 2) {
          ...
          if (zero_points) {
            range2scalezp<Tin, 4, signed_quant>(vmin_t[i], vmax_t[i], scale0_tt,
                                                v0_tt);
            range2scalezp<Tin, 4, signed_quant>(vmin_t[i + 1], vmax_t[i + 1],
                                                scale1_tt, v1_tt);
            zero_points[(scale_idx + i) >> 1] = Pack<false>(v0_tt, v1_tt);
          } else {
            range2scale<Tin, 4, signed_quant>(vmin_t[i], vmax_t[i], scale0_tt);
            range2scale<Tin, 4, signed_quant>(vmin_t[i + 1], vmax_t[i + 1],
                                              scale1_tt);
          }
        ...
        for (int32_t j = 0, input_idx_t = input_idx; j < row_size;
          ...
          for (int32_t i = 0; i < col_size; i += 2) {
            v0_tt =
              QuantizeV(src[input_idx_t + i], reciprocal_scale_t[i], zp_t[i]);
            v1_tt = QuantizeV(src[input_idx_t + i + 1],
                              reciprocal_scale_t[i + 1], zp_t[i + 1]);
  ...
  static void QuantizeColumnWisePackUnaligned(const Tin *src, Tin *scales,
                                              uint8_t *zero_points,
                                              uint8_t *dst, int32_t rows,
                                              int32_t columns,
                                              int32_t quant_block_size,
                                              MLAS_THREADPOOL *thread_pool) {
    // Thread block is [quant_block_size * 2, columns], so the packed bytes do
    // not cross threads.
    constexpr auto minf = std::numeric_limits<float>::lowest();
    constexpr auto maxf = std::numeric_limits<float>::max();
    auto row_thread_blk_size = quant_block_size * 2;
    auto num_row_thread_blk =
    ...
    MlasTryBatchParallel(
      ...
      [&](ptrdiff_t thread_blk_idx) {
        ...
        for (; row_idx < row_idx_end; row_idx += quant_block_size) {
          // per quant block row
          auto quant_row_size =
            std::min(quant_block_size, row_idx_end - row_idx);
          auto input_buffer_idx = input_idx;
          auto scale_buffer_idx = scale_idx;
          for (int32_t buffer_idx = 0; buffer_idx < columns;
            ...
            for (int32_t j = 0; j < quant_row_size;
                 ++j, input_idx_t += columns) {
              // TODO(fajin): use SIMD
            ...
            if (scale_buffer_idx & 1) {
              ...
              if (zero_points) {
                range2scalezp<Tin, 4, signed_quant>(vmin_t[0], vmax_t[0],
                                                    scale0_tt, v0_tt);
                zero_points[scale_buffer_idx >> 1] =
                  SetElem(v0_tt, 1, zero_points[scale_buffer_idx >> 1]);
              } else {
                range2scale<Tin, 4, signed_quant>(vmin_t[0], vmax_t[0],
                                                  scale0_tt);
              }
            ...
            // aligned zero points
            for (; scale_buffer_idx < scale_buffer_idx_end - 1;
                 col_idx += 2, scale_buffer_idx += 2) {
              v0_tt = v1_tt = BitsTraits<4, signed_quant>::kMid;
              if (zero_points) {
                range2scalezp<Tin, 4, signed_quant>(
                  vmin_t[col_idx], vmax_t[col_idx], scale0_tt, v0_tt);
                range2scalezp<Tin, 4, signed_quant>(
                  vmin_t[col_idx + 1], vmax_t[col_idx + 1], scale1_tt, v1_tt);
                zero_points[scale_buffer_idx >> 1] = Pack<false>(v0_tt, v1_tt);
              } else {
                range2scale<Tin, 4, signed_quant>(vmin_t[col_idx],
                                                  vmax_t[col_idx], scale0_tt);
                range2scale<Tin, 4, signed_quant>(
                  vmin_t[col_idx + 1], vmax_t[col_idx + 1], scale1_tt);
              }
            ...
            if (scale_buffer_idx < scale_buffer_idx_end) {
              ...
              if (zero_points) {
                range2scalezp<Tin, 4, signed_quant>(
                  vmin_t[col_idx], vmax_t[col_idx], scale0_tt, v0_tt);
                zero_points[scale_buffer_idx >> 1] =
                  SetElem(v0_tt, 0, zero_points[scale_buffer_idx >> 1]);
              } else {
                range2scale<Tin, 4, signed_quant>(vmin_t[col_idx],
                                                  vmax_t[col_idx], scale0_tt);
              }
            ...
            for (int32_t j = 0; j < quant_row_size;
              ...
              if (input_idx_t_start & 1) {
                v1_tt = QuantizeV(src[input_idx_t_start],
                                  reciprocal_scale_t[col_idx], zp_t[col_idx]);
                dst[input_idx_t_start >> 1] =
                  SetElem(v1_tt, 1, dst[input_idx_t_start >> 1]);
              ...
              // aligned output
              // TODO(fajin): use SIMD
              for (; input_idx_t_start < input_idx_t_end - 1;
                   col_idx += 2, input_idx_t_start += 2) {
                v0_tt = QuantizeV(src[input_idx_t_start],
                                  reciprocal_scale_t[col_idx], zp_t[col_idx]);
                v1_tt =
                  QuantizeV(src[input_idx_t_start + 1],
                            reciprocal_scale_t[col_idx + 1], zp_t[col_idx + 1]);
              ...
              if (input_idx_t_start < input_idx_t_end) {
                v0_tt = QuantizeV(src[input_idx_t_start],
                                  reciprocal_scale_t[col_idx], zp_t[col_idx]);
                dst[input_idx_t_start >> 1] =
                  SetElem(v0_tt, 0, dst[input_idx_t_start >> 1]);
              }
            }
  ...
  static void TransposeColumnWiseQuantizedPackAligned(
    ...
    const uint8_t
      *src_zero_points,   // [ceil(rows / quant_block_size), columns / 2]
    uint8_t *dst_weights, // [columns, ceil(rows / quant_block_size),
                          // ceil(quant_block_size / 2)]
    Tin *dst_scales,      // [columns, ceil(rows / quant_block_size)]
    uint8_t
      *dst_zero_points, // [columns, ceil(ceil(rows / quant_block_size) / 2)]
    int32_t rows, int32_t columns, int32_t quant_block_size,
    MLAS_THREADPOOL *thread_pool) {
    ORT_ENFORCE(columns % 2 == 0, "Columns must be multiple of 2");
    ...
    // weight transpose thread block is [dst_bytes_per_quant_blk, 2] on
    // dst_Transpose. Map to src it is [quant_block_size, 1]. Both in uint8_t.
    ...
    MlasTryBatchParallel(
      ...
      [&](ptrdiff_t thread_blk_idx) {
        ...
        auto row_thread_blk_idx =
          static_cast<int32_t>(thread_blk_idx / packed_col_size);
        auto col_thread_blk_idx =
          static_cast<int32_t>(thread_blk_idx % packed_col_size);
    ...
    // Transpose scales. Thread block is [row_quant_blk_num, 1] on
    // dst_Transpose.
    MlasTryBatchParallel(thread_pool, static_cast<ptrdiff_t>(columns),
                         [&](ptrdiff_t thread_blk_idx) {
                           auto col_thread_blk_idx =
                             static_cast<int32_t>(thread_blk_idx);
                           auto src_idx = col_thread_blk_idx;
                           auto dst_idx =
                             col_thread_blk_idx * row_quant_blk_num;
                           for (int32_t i = 0; i < row_quant_blk_num;
    ...
    if (src_zero_points) {
      ...
      // on dst_Transpose. Map to src it is [row_quant_blk_num, 1]. Both in
      // uint8_t.
      ...
      MlasTryBatchParallel(
        ...
        [&](ptrdiff_t thread_blk_idx) {
          ...
          auto src_idx = col_thread_blk_idx;
          auto src_end_idx =
            row_quant_blk_num * packed_col_size + col_thread_blk_idx;
  ...
  static void TransposeColumnWiseQuantizedPackUnaligned(
    ...
    const Tin *src_scales,          // [ceil(rows / quant_block_size), columns]
    const uint8_t *src_zero_points, // size of [ceil(ceil(rows /
                                    // quant_block_size) * columns / 2)]
    uint8_t *dst_weights,           // [columns, ceil(rows / quant_block_size),
                                    // ceil(quant_block_size / 2)]
    Tin *dst_scales,                // [columns, ceil(rows / quant_block_size)]
    uint8_t
    ...
    // weight transpose thread block is [dst_bytes_per_quant_blk, 1] on
    // dst_Transpose in uint8_t. Map to src it is [quant_block_size, 1] in int4.
    ...
    MlasTryBatchParallel(
      ...
      [&](ptrdiff_t thread_blk_idx) {
        ...
        auto row_thread_blk_idx =
          static_cast<int32_t>(thread_blk_idx / columns);
        auto col_thread_blk_idx =
          static_cast<int32_t>(thread_blk_idx % columns);
        ...
        for (; src_idx < src_end_idx - columns; ++dst_idx) {
          ...
          src1_t = GetElem(src_weights[(src_idx + columns) >> 1],
                           (src_idx + columns) & 1);
    ...
    if (src_zero_points) {
      // Transpose zero points. Thread block is [ceil(row_quant_blk_num / 2), 1]
      // on dst_Transpose in uint8_t. Map to src it is [row_quant_blk_num, 1] in
      // int4.
      auto dst_zp_row_num = (row_quant_blk_num + 1) / 2;
      MlasTryBatchParallel(
        ...
        [&](ptrdiff_t thread_blk_idx) {
          ...
          for (; src_idx < src_end_idx - columns; ++dst_idx) {
            ...
            src1_t = GetElem(src_zero_points[(src_idx + columns) >> 1],
                             (src_idx + columns) & 1);
...
void MlasBlockwiseQuantMetaShape(int block_size, bool columnwise, int rows,
                                 int columns, int &meta_rows, int &meta_cols) {
  switch (block_size) {
  case 16: {
    if (columnwise) {
      BlockwiseQuantizer<T, 16, qbits, true>::quantizeMetaShape(
        rows, columns, meta_rows, meta_cols);
    } else {
      BlockwiseQuantizer<T, 16, qbits, false>::quantizeMetaShape(
        rows, columns, meta_rows, meta_cols);
    }
    break;
  }
  case 32: {
    if (columnwise) {
      BlockwiseQuantizer<T, 32, qbits, true>::quantizeMetaShape(
        rows, columns, meta_rows, meta_cols);
    } else {
  ...
  case 64: {
    if (columnwise) {
      BlockwiseQuantizer<T, 64, qbits, true>::quantizeMetaShape(
        rows, columns, meta_rows, meta_cols);
    } else {
      BlockwiseQuantizer<T, 64, qbits, false>::quantizeMetaShape(
        rows, columns, meta_rows, meta_cols);
    }
    break;
  }
  case 128: {
    if (columnwise) {
      BlockwiseQuantizer<T, 128, qbits, true>::quantizeMetaShape(
        rows, columns, meta_rows, meta_cols);
    } else {
      BlockwiseQuantizer<T, 128, qbits, false>::quantizeMetaShape(
        rows, columns, meta_rows, meta_cols);
    }
    break;
  }
  case 256: {
    if (columnwise) {
      BlockwiseQuantizer<T, 256, qbits, true>::quantizeMetaShape(
        rows, columns, meta_rows, meta_cols);
    } else {
      BlockwiseQuantizer<T, 256, qbits, false>::quantizeMetaShape(
        rows, columns, meta_rows, meta_cols);
    }
    break;
  }
...
template <typename T, int qbits>
void MlasBlockwiseQuantizedShape(int block_size, bool columnwise, int rows,
                                 int columns, int &q_rows, int &q_cols) {
  switch (block_size) {
  case 16: {
    if (columnwise) {
      BlockwiseQuantizer<T, 16, qbits, true>::quantizedShape(rows, columns,
                                                             q_rows, q_cols);
    } else {
      BlockwiseQuantizer<T, 16, qbits, false>::quantizedShape(rows, columns,
                                                              q_rows, q_cols);
    }
    break;
  }
  case 32: {
    if (columnwise) {
      BlockwiseQuantizer<T, 32, qbits, true>::quantizedShape(rows, columns,
                                                             q_rows, q_cols);
    } else {
      BlockwiseQuantizer<T, 32, qbits, false>::quantizedShape(rows, columns,
                                                              q_rows, q_cols);
    }
    break;
  }
  case 64: {
    if (columnwise) {
      BlockwiseQuantizer<T, 64, qbits, true>::quantizedShape(rows, columns,
                                                             q_rows, q_cols);
    } else {
      BlockwiseQuantizer<T, 64, qbits, false>::quantizedShape(rows, columns,
                                                              q_rows, q_cols);
    }
    break;
  }
  case 128: {
    if (columnwise) {
      BlockwiseQuantizer<T, 128, qbits, true>::quantizedShape(rows, columns,
                                                              q_rows, q_cols);
    } else {
      BlockwiseQuantizer<T, 128, qbits, false>::quantizedShape(rows, columns,
                                                               q_rows, q_cols);
    }
    break;
  }
  case 256: {
    if (columnwise) {
      BlockwiseQuantizer<T, 256, qbits, true>::quantizedShape(rows, columns,
                                                              q_rows, q_cols);
    } else {
      BlockwiseQuantizer<T, 256, qbits, false>::quantizedShape(rows, columns,
                                                               q_rows, q_cols);
    }
    break;
  }
...
template void MlasBlockwiseQuantMetaShape<float, 4>(int block_size,
                                                    bool columnwise, int rows,
                                                    int columns, int &meta_rows,
                                                    int &meta_cols);

template void MlasBlockwiseQuantMetaShape<MLAS_FP16, 4>(int block_size,
                                                        bool columnwise,
                                                        int rows, int columns,
                                                        int &meta_rows,
                                                        int &meta_cols);

template void MlasBlockwiseQuantizedShape<float, 4>(int block_size,
                                                    bool columnwise, int rows,
                                                    int columns, int &q_rows,
                                                    int &q_cols);

template void MlasBlockwiseQuantizedShape<MLAS_FP16, 4>(int block_size,
                                                        bool columnwise,
                                                        int rows, int columns,
                                                        int &q_rows,
                                                        int &q_cols);

void MLASCALL MlasBlockwiseQuantizedBufferSizes(
  int qbits, int block_size, bool columnwise, int rows, int columns,
  size_t &q_data_size_in_bytes, size_t &q_scale_num_elements,
    ...
    case 16:
      if (columnwise) {
        BlockwiseQuantizer<float, 16, 4, true>::quantizedBufferSizes(
          rows, columns, q_data_size_in_bytes, q_scale_num_elements,
...
template <typename T, int qbits>
void MlasQuantizeBlockwise(uint8_t *dst, T *scales, uint8_t *zero_points,
                           const T *src, int block_size, bool columnwise,
                           int rows, int columns, int leading_dimension,
                           MLAS_THREADPOOL *thread_pool) {
  switch (block_size) {
  case 16:
    if (columnwise) {
...
template void MlasQuantizeBlockwise<float, 4>(
  uint8_t *dst, float *scales, uint8_t *zero_points, const float *src,
  int block_size, bool columnwise, int rows, int columns, int leading_dimension,
  MLAS_THREADPOOL *thread_pool);

template void MlasQuantizeBlockwise<MLAS_FP16, 4>(
  uint8_t *dst, MLAS_FP16 *scales, uint8_t *zero_points, const MLAS_FP16 *src,
  int block_size, bool columnwise, int rows, int columns, int leading_dimension,
  MLAS_THREADPOOL *thread_pool);

template <typename T, int qbits>
void MlasDequantizeBlockwise(T *dst, const uint8_t *src, const T *scales,
                             const uint8_t *zero_points, int block_size,
                             bool columnwise, int rows, int columns,
                             MLAS_THREADPOOL *thread_pool) {
  switch (block_size) {
  case 16:
    if (columnwise) {
...
template void MlasDequantizeBlockwise<float, 4>(float *dst, const uint8_t *src,
                                                const float *scales,
...
bool MlasQDQQuantizeBlockwise(const Tin *src, Tin *scales, uint8_t *zero_points,
                              uint8_t *dst, bool columnwise, int rows,
                              int columns, int quant_block_size,
                              MLAS_THREADPOOL *thread_pool) {
  if (columnwise) {
    if (zero_points) {
      BlockwiseQDQQuantizer<Tin, qbits, false>::QuantizeColumnWise(
        src, scales, zero_points, dst, rows, columns, quant_block_size,
        thread_pool);
      return false;
    } else {
...
template bool MlasQDQQuantizeBlockwise<float, 4>(
  const float *src, float *scales, uint8_t *zero_points, uint8_t *dst,
  bool columnwise, int rows, int columns, int quant_block_size,
  MLAS_THREADPOOL *thread_pool);

template bool MlasQDQQuantizeBlockwise<MLAS_FP16, 4>(
  const MLAS_FP16 *src, MLAS_FP16 *scales, uint8_t *zero_points, uint8_t *dst,
  bool columnwise, int rows, int columns, int quant_block_size,
  MLAS_THREADPOOL *thread_pool);
...
void MlasQDQTransposeBlockwiseQuantized(
  const uint8_t *src_weights, const Tin *src_scales,
  const uint8_t *src_zero_points, uint8_t *dst_weights, Tin *dst_scales,
  uint8_t *dst_zero_points, bool columnwise, int rows, int columns,
  int quant_block_size, MLAS_THREADPOOL *thread_pool) {
  if (columnwise) {
    BlockwiseQDQQuantizer<Tin, qbits, signed_quant>::
...
template void MlasQDQTransposeBlockwiseQuantized<float, 4, true>(
  const uint8_t *src_weights, const float *src_scales,
  const uint8_t *src_zero_points, uint8_t *dst_weights, float *dst_scales,
  uint8_t *dst_zero_points, bool columnwise, int rows, int columns,
  int quant_block_size, MLAS_THREADPOOL *thread_pool);

template void MlasQDQTransposeBlockwiseQuantized<float, 4, false>(
  const uint8_t *src_weights, const float *src_scales,
  const uint8_t *src_zero_points, uint8_t *dst_weights, float *dst_scales,
  uint8_t *dst_zero_points, bool columnwise, int rows, int columns,
  int quant_block_size, MLAS_THREADPOOL *thread_pool);

template void MlasQDQTransposeBlockwiseQuantized<MLAS_FP16, 4, true>(
  const uint8_t *src_weights, const MLAS_FP16 *src_scales,
  const uint8_t *src_zero_points, uint8_t *dst_weights, MLAS_FP16 *dst_scales,
  uint8_t *dst_zero_points, bool columnwise, int rows, int columns,
  int quant_block_size, MLAS_THREADPOOL *thread_pool);

template void MlasQDQTransposeBlockwiseQuantized<MLAS_FP16, 4, false>(
  const uint8_t *src_weights, const MLAS_FP16 *src_scales,
  const uint8_t *src_zero_points, uint8_t *dst_weights, MLAS_FP16 *dst_scales,
  uint8_t *dst_zero_points, bool columnwise, int rows, int columns,
  int quant_block_size, MLAS_THREADPOOL *thread_pool);
