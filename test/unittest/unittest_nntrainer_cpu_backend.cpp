...
template <typename T>
static inline std::vector<T> generate_random_vector(size_t size, float min_val = -0.5F, float max_val = 0.5F){
...
TEST(nntrainer_cpu_backend_standalone, q4_K_GEMM_3B_3072x3072x512) {
    ///@note A(M, K) * W.T(N, K) = (M, N)
    ///@note A(sizez, sizex) * W.T(sizey, sizex) = (sizez, sizey)
...
TEST(nntrainer_cpu_backend_standalone, q4_K_GEMM_3B_512x3072x512) {
    ///@note A(M, K) * W.T(N, K) = (M, N)
    ///@note A(sizez, sizex) * W.T(sizey, sizex) = (sizez, sizey)
...
TEST(nntrainer_cpu_backend_standalone, q4_K_GEMM_3B_2048x2048x512) {
    ///@note A(M, K) * W.T(N, K) = (M, N)
    ///@note A(sizez, sizex) * W.T(sizey, sizex) = (sizez, sizey)

    const unsigned int M = 512; // = sizez
    const unsigned int K = 2048; // = sizex
    const unsigned int N = 2048; // = sizey
...
TEST(nntrainer_cpu_backend_standalone, q4_K_GEMV_1B_8192x2048x1) {
    ///@note A(M, K) * W.T(N, K) = (M, N)
    ///@note A(sizez, sizex) * W.T(sizey, sizex) = (sizez, sizey)
...
TEST(nntrainer_cpu_backend_standalone, DISABLED_q4_K_GEMM_GAUSS_3B_1024_3072_3072) {
    ///@note A(M, K) * W.T(N, K) = (M, N)
    ///@note A(sizez, sizex) * W.T(sizey, sizex) = (sizez, sizey)
...
TEST(nntrainer_cpu_backend_standalone, q4_K_GEMV_1_3072_3072) {
    ///@note A(M, K) * W.T(N, K) = (M, N)
    ///@note A(sizez, sizex) * W.T(sizey, sizex) = (sizez, sizey)
...
TEST(nntrainer_cpu_backend_standalone, DISABLED_sgemm_standalone_512128512) {
    ///@note A(M, K) * W.T(N, K) = (M, N)
    ///@note A(sizez, sizex) * W.T(sizey, sizex) = (sizez, sizey)

    const unsigned int M = 512; // = sizez
    const unsigned int K = 128; // = sizex
    const unsigned int N = 512; // = sizey
...
TEST(nntrainer_cpu_backend_standalone, DISABLED_sgemm_standalone_128512512) {
    ///@note A(M, K) * W.T(N, K) = (M, N)
    ///@note A(sizez, sizex) * W.T(sizey, sizex) = (sizez, sizey)

    const unsigned int M = 128; // = sizez
...
TEST(nntrainer_cpu_backend_standalone, DISABLED_q4_K_GEMV_1024x1_3072_3072) {
    ///@note A(M, K) * W.T(N, K) = (M, N)
    ///@note A(sizez, sizex) * W.T(sizey, sizex) = (sizez, sizey)

    const unsigned int M = 1024; // = sizez
    const unsigned int _M = 1; // = sizez
    const unsigned int K = 3072; // = sizex
    const unsigned int N = 3072; // = sizey
    unsigned int pseudo_batch = M;
    ...
    // Step3. Run GEMM! (Online activation quantization + kernel routine + return float)
    t1 = high_resolution_clock::now();
    for (unsigned int pb = 0; pb < pseudo_batch; ++pb){
        nntrainer::gemm_q4_K(_M, N, K, lhs_ptr + pb * K, K, (void*) repacked_qWeight.data(), N, dst_ptr + pb * N, N);
    }
    t2 = high_resolution_clock::now();
    dt = duration_cast<nanoseconds>(t2 - t1);
    std::cout << "gemm_q4_K : " << dt.count()
...
TEST(nntrainer_cpu_backend_standalone, q4_K_GEMM_1024x3072_3072) {
    ///@note A(M, K) * W.T(N, K) = (M, N)
    ///@note A(sizez, sizex) * W.T(sizey, sizex) = (sizez, sizey)
...
TEST(nntrainer_cpu_backend_standalone, q4_K_GEMM_256x256_256) {
    ///@note A(M, K) * W.T(N, K) = (M, N)
    ///@note A(sizez, sizex) * W.T(sizey, sizex) = (sizez, sizey)

    const unsigned int M = 256; // = sizez
    const unsigned int K = 256; // = sizex
    const unsigned int N = 256; // = sizey
    unsigned int pseudo_batch = M;
    ...
    std::cout << "REF MATRIX\n";
    print_matrix<float>(ref_dst_ptr, M, N); // for it is a vector
    std::cout << "DEBUG MATRIX\n";
    print_matrix<float>(dst_ptr, M, N);
