...
std::vector<int64_t> const prefix{1648, 19447, 39, 234};
//"## Input: "

std::vector<int64_t> const postfix{212, 9384, 61438, 611, 27,
                                   212, 1648, 5231,  39};
//"\nPlease summarize this.\n## output:";

int const NUM_PREFIX = 4;
int const NUM_POSTFIX = 9;
...
void run(std::string text, bool apply_temperature) {
  ...
  auto _input = tokenizer.encode(text);

  unsigned int _len = _input.size();

  std::cout << "### Input : (" << _len << " words)" << std::endl;

  unsigned int num_allow_str =
    MAX_SEQ_LEN - (NUM_PREFIX + NUM_POSTFIX + NUM_TO_GENERATE);

  std::vector<int64_t> init_input(prefix.begin(), prefix.end());
  ...
  for (unsigned int i = 0; i < NUM_POSTFIX; ++i) {
    init_input.push_back(postfix[i]);
  }
  ...
  if (optimize) {
    ...
    for (unsigned int i = input_len + 1; i < input_len + NUM_TO_GENERATE; ++i) {
      ...
      // ids = std::distance(
      //   output_interval[0],
      //   std::max_element(output_interval[0], output_interval[0] +
      //   NUM_VOCAB));
